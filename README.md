...
同一个 FastAPI 挂 `/mcp/`（Streamable HTTP），独立 Bearer 鉴权，不复用登录 cookie。九个工具：知识四件 `search_published` 检索已发布、`get_asset` 取已发布正文（可取历史已发布版；第 120 刀起不回对象键，媒体资产以 has_media 指引 `get_asset_media`）、`register_asset` 带正文登记（来源服务端定值；第 120 刀起可带 `knowledge_gap_id` 挂缺口补文档）、`export_published` 全量导出已发布；活状态只读三件 `get_product` / `get_stock` / `get_order_status`（第 99 刀起，包的是与客服同一套查询函数，订单返回不含顾客联系方式）；运营飞轮两件 `list_knowledge_gaps`（第 120 刀：看拒答缺口，外部 Agent 补文档闭环的入口）与 `get_asset_media`（第 120 刀：已发布图片/视频字节 base64 返回，多模态 Agent 直接可消费）。**没有 publish 这个工具**——发布权只在治理台操作者手里。MCP `export_published` 导出已发布正文数据包；微调数据集（SFT 形态，只出已发布对话资产的人工确认问答对，逐条带资产版本血缘）经治理台 `POST /api/exports/sft` 导出——本产品不做训练。
...
- **七工具**（第 99 刀/ADR 0057：知识四件 + 活状态只读三件；没有 publish——发布只属于治理台操作者）：
  ...
  | `get_asset(asset_id, version?)` | 取已发布资产正文与元数据；不传 version=当前指针版，传 version=历史已发布版；待人洗/已接入一律拒绝。第 120 刀起不回对象键（0052 键不出门），媒体资产回 `has_media` 指引 `get_asset_media` |
| `get_asset_media(asset_id, version?)` | 已发布图片/视频资产的字节 base64 返回（多模态消费面，第 120 刀）；非媒体拒绝 |
| `list_knowledge_gaps(status?, limit?)` | 知识缺口列表（运营飞轮入口，第 120 刀）：open/resolved、按被问热度降序，question 出口打码 |
...
## 自媒体内容套件（第 98 刀，ADR 0055；第 115 刀配图语义重构）

**素材中心选模板（站内投放/小红书笔记体/短视频口播稿）→ 请求内生成文案 + 可选配图 → 双闸质检（规则四条 + LLM 事实性核对）→ 待抽检 → 操作者通过 → 双资产登记（文案=素材资产、配图=图片资产）→ 照常机洗人洗发布。**

- **三模板只是 prompt 参数**（不是 Agent、不加 planner）：`POST /api/material/tasks` 收 `template`（`station`/`xhs`/`short_video`，默认站内=17 刀形态）与 `with_image`。**第 115 刀 W16**：三模板的风格段显式化——小红书按爆款结构（标题 ≤20 字 + 钩子公式三选一 + SCQA 四段 + emoji 段落标记 + 3-5 个话题标签按「泛流量+垂直+长尾」配比 + 禁极限词）；站内标题 ≤16 字含商品名；口播开场 3 秒钩子、每句 8-15 字。运营编排（ops 的 `gen_material`）是另一条独立通道。
- **配图按模板三路分派（第 115 刀 W15，语义各归各）**：
  - **站内 = 美化产品图（AI 编辑）**：取该商品的**真实图片资产**（已发布优先）→ `imggen.edit_image` 指令式图像编辑（`IMGGEN_EDIT_MODEL`，默认 `Qwen/Qwen-Image-Edit-2509`）——商品主体来自原图而非模型想象（指令含保留项申明「外观/颜色/材质/细节完全不变」+ 光照一致 + 禁文字）。参考图落 `image_reference_asset_id`（迁移 0035，任务详情展示「参考实拍图 A-xxxx」）。**商品没有图片资产 = 诚实跳过**（`image_status=skipped_no_image`，不退回文生图冒充——探针实证过参考图里没有商品时模型会凭空画一个）；先传图再点「补生成配图」（`POST /api/material/tasks/{id}/retry-image` 只重跑配图步，不动已过双闸的文案）。
  - **小红书 = 封面文字卡（确定性流水线，无 AI）**：按文案标题程序化渲染 1080×1440 大字报卡（六色板按任务 id 取模、白色圆角卡、降字号自动换行 ≤3 行、商品名关键词高亮色条、类目 chip + 底部小字；Pillow + fonts-noto-cjk）——**同输入逐字节可复现**，不需要 IMGGEN key 也不需要商品图。
  - **口播 = 文生图背景图（98 刀原语义不变）**：背景不含商品主张，文生图成立；无 key 诚实跳过。
...
- **配图 = 素材成品非知识证据**（分层红线）：配图字节按魔数复验，抽检前暂存对象存储 `material/` 前缀（不是资产，任务详情可预览 `GET /api/material/tasks/{id}/image`）；抽检通过**一次登记双资产**——文案=material 资产、配图=独立 image 资产（来源=`素材生成`、标题 `{商品} · {模板名}配图`、挂同商品、图片描述预填文案首句——VLM 看图草稿可用时优先，人洗可改）。生成图不回写商品规格、不冒充实拍证据。
- **IMGGEN 四 env**（表同上 ASR/VLM 风格）：

| 变量 | 说明 |
| --- | --- |
| `IMGGEN_API_KEY` | 图像服务密钥；**为空时不建客户端、不发请求 = 配图步诚实跳过**（任务不 fail；小红书文字卡不受影响）。密钥只写本机 `.env` |
| `IMGGEN_BASE_URL` | OpenAI 兼容端点（默认 `https://api.siliconflow.cn/v1` 硅基流动） |
| `IMGGEN_MODEL` | 文生图模型（默认 `black-forest-labs/FLUX.1-schnell`，口播背景图在用） |
| `IMGGEN_EDIT_MODEL` | 指令式图像编辑模型（第 115 刀，默认 `Qwen/Qwen-Image-Edit-2509`，美化产品图在用；同端点 `image` 参数形态，编辑超时 180s 独立预算） |

- **与 ASR/VLM 的 409 刻意不同级**：转写/打分没有本地兜底、无 key 就没有产物（fail-closed 拒绝）；配图是增值项不是任务本体——无 key 跳过、生成失败也只记 `image_status=failed`（不 fail 任务；待抽检任务可「补生成配图」，整任务重试也可再要图）。双闸没过线不跑配图步（不给废文案产图）。`GET /api/material/imggen/status` 供前端禁用「生成配图」开关（小红书文字卡不需要 key，开关对它恒可用）。
- **Out**：批量任务；编辑模型多图参考（image2/image3）；封面卡 emoji 图形化。

## 内容成片（第 98b 刀 ADR 0056；第 118 刀 W18 质量升级）

**素材中心「内容成片」页签选商品+模板 → plan 同步选材+排版（该商品已发布的切片/图片/文案要点，15-60s 时间线）→ 预览成片 + 剪映草稿 zip 双层产物 → 人审改（下草稿精修或直接看预览）→「确认登记」把文案要点过双闸复用登记 material 资产进治理。AI 排版、人上市。**

**第 118 刀（W18，Owner 验收「就是几个字幕+普通解说」反馈）——程序化管线拉满，不上视频生成模型**（调研裁决：视频生成 ¥0.8-1.0/秒且商品保真不可控；行业主流=素材混剪+Ken Burns+TTS+字幕+BGM）：
- **广告脚本结构**：口播稿=「黄金 3 秒钩子（悬念提问式，如『{商品}，到底值不值？』）→ 卖点段 → CTA（『点击主页，把它带回家』）」完整广告词；时间线首尾对应钩子卡/CTA 卡（`role` 标注，钩子/CTA 是结构卡——固定时长不参与补齐拉长、不进 publish 文案面）。
- **Ken Burns 运镜**：图片项 zoompan 交替推近/拉远（2x 画幅内缩放，不篡改商品）。
- **文案卡视觉**：双色渐变底 + role 配色（钩子暖字/卖点白字/CTA 高亮底条）+ 左上角商品名小标——与素材中心封面文字卡同一套视觉语言。
- **口播垫乐**：程序化轻和弦 pad（音量 18%，行业纪律 15-20% 不盖人声），只陪口播轨（无 TTS 仍纯无声）。
- **水印取消**（Owner 裁决，ADR 0056 红线①修订）：画面全部来自真实素材的程序化剪辑、无 AI 生成画面——「AI 生成」角标属过度声明；TTS 口播的标注责任在发布环节。
...
- **红线四条**（ADR 0056；①已由第 118 刀修订撤销）：①~~AIGC 水印常驻~~（Owner 裁决移除——真实素材程序化剪辑，无 AI 生成画面）；②选材白名单=只取已发布+自有来源（本仓资产面天然满足；第 116 刀起非可播字节如旧时间码文本切片在选材层跳过）；③publish 文案双闸复用；④数字人/声音克隆 Out——TTS voice 用厂商预置音色。
