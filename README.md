...
- **切片「上传即绑定」是一次性的**：上传源录像时只会绑「尚无源录像的 pending 候选」；演示库若已绑过（第 46 刀验收跑过），再上传会如实回执「当前没有可绑定的待拣候选」，此时拣选切的是**已绑的那份录像**。要重演「上传→绑定」这条路径，先把演示库复位：

  ```bash
  docker compose exec db psql -U suite -d suite     -c "UPDATE clip_candidates SET recording_id = NULL WHERE status = 'pending';"
  ```

  （源录像本身会留在对象存储里成为孤儿——这是有意的最小实现：录像是切片模块自有的输入源，没有删除/替换端点。重绑/替换是待裁决的下一刀。）
