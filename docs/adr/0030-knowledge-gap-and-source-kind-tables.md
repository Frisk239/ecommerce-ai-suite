...
实施澄清（审计刀 2 补记，2026-09-08）：`gap_id` 运行时返回按通道分叉——操作者预览通道 `complete` 事件照带（前端芯片跳治理台缺口 tab，本文原口径）；**顾客通道（0021）不带回**（载荷白名单，chat_engine 的 `expose_gap_id` 一参）：顾客不进控制台，内部缺口 id 对其无意义且属最小暴露。`fallback` 标记（第 7 刀）同为运行时口径，两通道都带（降级诚实标注对顾客同样适用）。
