...
| `service_sessions` | CONTEXT「会话=客服里进行中的对话…结束后由操作者回流登记为资产（种类=对话）」；ADR 0021（预览与顾客接口同一引擎） | id、status（active/closed/registered——运行态流转）、created_at、closed_at；**运行态实体，不是第三个中台对象**（词条 Avoid）；回流登记后状态置 registered 并指向登记出的资产 |
