...
## 修订（第 120 刀，2026-09-18）——工具七件 → 九件（覆盖走查驱动）

Owner 以外部 Agent 身份真协议走查旅程后裁决补齐两块缺口：

1. **`list_knowledge_gaps(status?, limit?)`（运营飞轮入口，只读）**：走查实证
   外部 Agent 看不到「顾客被拒答了什么」——缺口是飞轮起点（0024），没有它
   外部 Agent 无法参与「拒答 → 补口径 → 发布解决」的闭环。配套
   `register_asset` 增加 `knowledge_gap_id` 可选参数（同治理台 0024 语义：
   缺口须 open 且未挂其他补文档；发布事务内才 resolve——发布权仍不在工具面）。
2. **`get_asset_media(asset_id, version?)`（多模态消费面，只读）**：走查实证
   外部 Agent 拿不到商品图/切片画面（顾客通道有 media_citations，MCP 没有）。
   已发布媒体字节 base64 返回，多模态 Agent 直接可消费。
3. **`get_asset` 不再返回 `object_key`**（走查揪出的泄漏：对象键是存储内部
   坐标，外部拿到没有用途还扩泄漏面——0052「键不出门」纪律本就该罩住本
   出口）；改回 `has_media` 布尔指引媒体工具。

刻意不加的（仍是红线）：publish、治理写操作（人洗/抽检）、素材任务/切片/
成片工作台、考核/工单/会话列表——外部 Agent 的边界=「读已发布 + 读活状态 +
登记回流」，发布权不可外包（0001/0005）不动。
