...
## 修订（第 118 刀，2026-09-18）——Owner 两项裁决

1. **红线①撤销：预览与草稿不再带「AI 生成」水印**。Owner 裁决理由：本引擎
   的画面全部来自**真实素材**（已发布切片/商品图/洗帧图）的程序化剪辑，不
   引入 AI 生成画面——「AI 生成」角标属过度声明；TTS 合成口播的标注责任在
   发布环节（剪映精修/人工上市），预览与草稿不是对外交付面。原「水印不可
   配置关闭」的约束随特性一并移除（非改为可配置）。
2. **成片质量升级（W18，程序化管线，不上视频生成模型）**：广告脚本结构
   （片头钩子卡 + 片尾 CTA 卡，确定性模板句，`TimelineItem.role`）；
   Ken Burns 运镜（图片项 zoompan 交替推近/拉远）；文案卡双色渐变底 +
   role 配色 + 商品名小标；口播垫乐（程序化和弦 pad，音量 18%，只陪口播轨
   ——「无 TTS=纯无声」语义不变）。决策依据（同日调研）：视频生成模型
   成本 ¥0.8-1.0/秒（30s 成片 ¥24-30）且商品保真不可控（开源同类 A/B 实测
   「凭空长手、商品被挤出画面」），行业主流带货管线=素材混剪+Ken Burns+
   TTS+字幕+BGM，全程序化可实现。
