...
平行工作流：控制台观感与交互见 `docs/ui-ux-plan.md`。**不改本文件刀序。** 第 43 刀仪表落地前建议先做方案包 UX-E（总览/资产去等宽统计卡），避免再叠磁贴。最小可插刀是 UX-A+UX-B（工作队列 + 只读证据视图）。
...
- 会话结束（操作者回流时或顾客关闭）触发 1–5 星评分（顾客面一次性提示；Chatwoot 标配形态）；挂到会话与缺口分析（43 刀仪表加 CSAT 卡）。**订正（第 48/80 刀）**：顾客关闭事件已由第 80 刀提供（ended 终态），但评分形态维持**常驻评分条**（点星即提交、评过可改、结束后仍开放），不做触发式一次性提示——48 刀回退点在 80 刀兑现的是「结束后可评分」而非弹窗形态；偏离裁决见 `docs/progress/session-end-80-intake.md` Out 清单。
