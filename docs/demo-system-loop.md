...
外加证据收口在途回路（100–103，见文末）。每幕 = 操作步骤 + 预期画面 + 对应主张 +
...
- **在途幕**：第四阶段证据收口四刀（100 OFF 订正/101 评测扩容+embedding 门/102 生成 judge/103 压测）——交付后移栏。
...
操作步骤：
1. 资产页找 **A-501**（显示器商品图·实拍帧；第 110 刀重灌后的幕 2 素材）：
   版本列 v1 是旧图（咖啡胶囊照）/v2 是真图+VLM 复核描述/v3 是描述二洗
   （抹掉通用颜色词的检索净面）。
2. 开修订（`POST /api/assets/501/revisions`）→ PUT 新字节 → 机洗 VLM 出
   草稿 → 人洗 PATCH「图片描述=…」→ 发布 → 指针前移（110 刀实录 v1→v2→v3）。
3. 问「有带支架的显示器吗」→ citations 带 **{501, version_no=当前指针版}**
   ——引用跟随指针（旧实录：会话 #349/350/351 修订前引 501·v1+503·v2）；
   A-503 已在 110 刀退役（空标题重复份），实测引用归 A-501 当前版。
4. 历史版不丢：MCP `get_asset(501, version=1)` 仍可取（读权威含历史版）。

预期画面：A-501 详情「版本」列 v1/v2/v3 多行，线上版本=最新版；问句引用角标
跟随最新版。

可指测试：`apps/api/tests/test_lineage_integration.py`（版本链/指针前移）、
`test_image_integration.py`（图片描述确认→索引）。

**演示提示**：开 A-501 详情页对着版本列表讲（v1 假图 → v2 真图 → v3 描述二洗
即 110 刀实况）；再切到会话 #350 看引用角标。
...
**演示提示**：现成例子 **A-501**（显示器商品图·实拍帧，线上 v3）与
A-502（待发布实拍帧，线上 v2；110 刀同批换真图）。
...
2. 命中已发布图片资产时双引用直出：citations
   ...
   （会话 #350/#351 实录：**双图直出**；两图分别由 A-501/A-503 提供，
   第 110 刀后 A-503 退役、A-502 接显示器第二图——重演时引 A-501 当前版
   + A-502 当前版）。
...
东西也诚实（十一）→ 顾客体验连续（十二）→ 微调数据可溯源（幕 14）。
审计 19 后全部已实现；在途=证据收口（100–103）。
