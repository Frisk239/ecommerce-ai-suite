# 体系闭环演示手册（第 96 刀；第 111 刀同步第七阶段终态）

给「我自己用用」的演示剧本：按体系演进链顺序编排 **15 幕已实现回路**（十二主幕 +
幕 13/14 数据面 + 幕 15 W3 元问题回声），外加证据收口在途回路（100–103 已交付，
见文末）。每幕 = 操作步骤 + 预期画面 + 对应主张 + 可指测试文件。所有素材都在
演示库（db 5433）里真实发生过至少一次——实录出处见
`docs/research/real-usage-log.md`（Ⅰ=第 92 刀，Ⅱ=第 96 刀）。
...
# 只报告探针噪音（空会话/探针资产/素材失败任务/成片残留/录像探针）；要清须显式 --apply
# 注意：第十幕要用「素材任务 failed」当证据就别 --apply（清行即成，不可逆）
```

浏览器开 `http://localhost:5173`（操作者台：会话/资产/缺口/审计四个 tab）；
顾客通道在客服页右下角小组件（95 刀起可嵌入、可续接）。**店铺宿主页**
（第 111 刀 W1 升级）在 `http://localhost:5173/storefront.html`——导航/四张真
商品图/政策与营业时间页脚，整页仍只多一行 `embed.js`（真店接入的形态本身）。
...
- **拒答话术**（第 108B 刀 W4 四段式）：承认边界 + 不编造理由 + 转人工回执 +
  顾客行动选项——「抱歉，这个问题我暂时没有查到可靠的资料——不想随便编一个
  答案误导您。已为您转人工处理（工单 H-xxxx），工作时间会在 4 小时内回复您。
  您也可以在下方留言，或者换个说法再问我一次。」（旧「已发布资产」内部术语已清零）。
...
## 第五幕：图片治理（上传 → 描述 → 发布 → 问句命中 + 人洗护栏）

**主张**：图片不是二进制黑箱——描述是它的检索文本面；VLM 只出草稿，
人确认生效（ADR 0051）；未确认的描述不进索引。**人洗时还有第二眼**
（第 111 刀护栏，W7 防复发）：PATCH 描述后后端调 VLM 独立描述做关键词交集，
响应附注回前端亮徽章——交集 ≥2 词 ✓一致、0–1 词 ⚠请复核，**只警示不阻止**
（人仍是最终裁决者）；VLM 未配置/失败/开关关=跳过复核（fail-open 到人洗兜底）。
...
2. 无 VLM key：图片描述=弃权（不写假值）；有 key：VLM 草稿落 extracted
   （source=machine）。
3. 人洗 PATCH 写/确认描述 → **响应带复核附注**：资产页「图片描述」面板上出现
   `✓ VLM 复核一致（关键词交集 N 词）` + VLM 独立描述原文；若描述与画面明显
   不符则 `⚠ VLM 复核疑似不符，请对照画面再核一遍`——字段照常保存，可改写后
   再存一次刷新徽章（`IMAGE_VERIFY_ON_WASH=false` 可整体关掉省调用）。
4. 发布 → 切块=描述文本（不是二进制）。
5. 顾客问图片内容词（如「有带支架的显示器吗」）→ 命中带引用。

预期画面：资产详情「图片描述」字段从草稿态到确认态，确认后徽章亮起；
问句回答引用该图。

可指测试：`apps/api/tests/test_image_integration.py`（94a 全契约：
草稿不进索引/409/商品素材聚合）、`test_image_verify_guard.py`（111 刀护栏四态：
通过/疑似不符不阻止/key 空跳过/非图片不触发 + 开关与打码）。

**演示提示**：现成例子 **A-501**（显示器商品图·实拍帧，线上 v3；
110 刀真图+描述二洗）与 A-502（待发布实拍帧，线上 v2；110 刀同批换真图、按
「不增块」口径无描述）；**A-516/A-517**（110 刀补充的耳机/键盘商品图，线上 v2）
在商品页素材段与 `storefront.html` 都能看到同一张实拍图。
...
### 幕 15：会话元问题回声（第 108B 刀 W3 真缺陷修复，可选加演）

- 操作：客服页先问一句真问题（如「你们几点上班」→ 答 9:00–21:00），紧接着问
  「我的上一个问题是什么」。
- 预期画面：**模板回声**「您上一条问的是『你们几点上班』。」——不调 LLM、
  不落知识缺口、不转人工（首问就问则答「这是我们对话的第一条消息」）。
- 主张：元问题/闲聊不是知识问题——修复前它走拒答 + 转人工 + **污染缺口池**
  （G-148 实证），现在走会话记忆快路径（与转人工/订单号/库存同族词表）。
- 可指测试：`apps/api/tests/test_meta_echo.py`（回声/首问边界/**不落缺口**断言）。
- 演示提示：换说法（「我刚才问了什么」「再说一遍」）都命中；缺口 tab 里查不到
  该问——这正是「不该落的缺口不落」的现场证据。
...
**第七阶段（108–111 数据治理+护栏）收官后的现场增量**：第五幕人洗多一道
VLM 复核徽章（✓一致/⚠请复核，护栏不阻止人）；幕 15 为 W3 元问题回声（可加演）；
店铺宿主页 `storefront.html` 已升级为导航+四张真商品图+政策页脚的完整观感
（W1）；拒答话术为 W4 四段式（内部术语清零）。终态基线 88.5/97.0、拒答 86.7、
误拒 0.0（`docs/research/rag-eval-report.md` 第七阶段收官节）。
