...
上一刀：**第 56 刀 类目别名与口语问价覆盖**（`feat/category-alias-56`，closeout 见 `docs/progress/category-alias-closeout.md`）——修审计刀 11 C-P1-3（顾客口语提问三条全拒答），**无新表无迁移**：①`CATEGORY_ALIASES`（笔记本/笔电/手提电脑→笔记本电脑、手机→智能手机、平板→平板电脑、电视→电视机，**保守**：无歧义的短称才收）；②报价意图词补 `怎么卖/怎么买/卖多少/什么价/啥价/多钱`；③提纯词表改**只收原子词、长词在前**（复合词会切掉后缀留残渣，「什么价格」曾被「什么价」吃掉剩「格」——评测集里就有这例）；④库存**类目聚合**（`_category_stock` + `render_stock_answer` 类目行 + `_run_stock_ask` 事实分支放宽：「你们有笔记本吗？」→「笔记本电脑共 34 件，其中有货 34 件，库存合计 1653 件」）；⑤库存路由器补 `有没有/有…吗`（**只做路由**，查不到即回落——「有现货吗」实测仍拒答不误答）。实测：`笔记本多少钱？`/`笔记本电脑怎么卖？`/`手机多少钱？` 三条全部答出类目价，服务问仍被纯度闸挡住。集成 937→941 passed。审计刀 11（PR #84）、第 55 刀数据集逐个可见（PR #83）、第 54 刀 widget 宿主站点（PR #82）、第 53 刀 CSAT 留言可回溯（PR #81）、第 52 刀类目问价（PR #80）、第 51 刀工作队列筛选（PR #78）均已合并 main。
