...
上一刀：**审计刀 12**（`feat/audit-12`，closeout 见 `docs/progress/audit-12-closeout.md`）——三路并行只读审计第 57–60 刀：**P0×3 实修**（①**第 58 刀的闸把「部分答对 + 免责句」整条吞成拒答**——内置建议问句「怎么退货？」2/3 概率变拒答+落缺口+建工单，改成**按句判 + 摘句作答**（还有实质内容就照常答）②闸的误判面（「配料信息中不包含防腐剂」等被当免责句）→ 主语收紧为证据类名词 + 宾语紧贴动词 ③第 60 刀的「订正」自己写错事实（`pos-017` top-1 是 asset 3 不是 13；那 1.3pp 只来自 `pos-008` 一条）→ 四处订正）+ **P1×6 实修**（「未提供/未说明」漏检补齐谓语面／缺口收口从目录路径泛化到**任何答上的出口**（库存/RAG 已答的缺口现会关）／折叠行补齐第 9 列并恢复行尾「重试机洗」／筛选只命中导入货时自动展开／`fallback_reason` 按 gap_id 白名单只给操作者通道／「缺口照落」钉子从恒真改成真断言）+ P2 六条（评测 runner 加**过期期望守卫**、CONTEXT 清掉横跨 6 刀的「widget origin 待裁决」矛盾、测试计数订正、第 59 刀「同列」措辞）。集成 945→957 passed；大集评测复跑 70.0/75.0（守卫生效）。第 60 刀 golden 过期期望维护（PR #89）、第 59 刀工作队列折叠（PR #88）、第 58 刀弱命中收口（PR #87）、第 57 刀会话详情（PR #86）、第 56 刀类目别名（PR #85）均已合并 main。
