...
上一刀：**第 14 刀库存工具**（`feat/stock-tool`，closeout 见 `docs/progress/stock-tool-closeout.md`）：只读 `get_stock`（ADR 0037：stock=商品 mock 字段；词表分派在代码；LCS 商品匹配；五分支回答——0 件是事实回答，NULL/未命中/故障转人工不缺口）复用 tool 事件与工具条；_Avoid_「用规格文档答有没有货」闭环。前十三刀已合并 main。

当前阶段：工程刀。下一刀：**第 15 刀**（候选：评测集/素材，开刀时裁决）；**第 15 刀后触发审计刀 3**。
...
## 第 14 刀：库存工具（已交付，`feat/stock-tool`）
...
| 刀 | 约束 |
| --- | --- |
| 评测集 | golden conversations + policy edges 进 CI（0027 评测集不是考核）；第 15 刀候选 |
| 素材 / 切片 / 考核 | 有接待飞轮和 MCP 证据后再进队。质检、候选不是资产已锁。不做微调（0028） |
| **审计刀 3** | **第 15 刀后触发**（候看：工具共享缝 45 行重复、词表误伤面、第 12/13 刀债务清单） |
