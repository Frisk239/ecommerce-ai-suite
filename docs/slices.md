...
上一刀：**第 60 刀 golden 过期期望维护**（`feat/golden-refresh-60`，closeout 见 `docs/progress/golden-refresh-closeout.md`）——把第 58 刀记的 1.3pp 评测漂移查清：`golden_large.json` 里 2 条（`pos-008`/`pos-017`）期望的 `13/v3` 被审计刀 11 的治理订正（v3→v4 对齐退货政策）取代，runner 比对 `(asset_id, version_no)` 即记未命中。把两条版本指到当前已发布版后复跑——**基线逐位恢复（positive 70.0/75.0、overall 65.0/75.0）**，证实漂移全部是过期期望而非检索回归；评测报告已加「订正」小节（含「发布类治理动作后要核对 golden 版本期望」的流程缺口）。**无代码改动**，集成 945 passed。第 59 刀工作队列导入货折叠（PR #88）、第 58 刀弱命中收口（PR #87）、第 57 刀会话详情（PR #86）、第 56 刀类目别名（PR #85）、审计刀 11（PR #84）均已合并 main。**下一刀=审计刀 12**（覆盖第 51–60 刀）。
