...
上一刀：**第 61 刀 演示库探针清理**（`feat/demo-reset-61`，closeout 见 `docs/progress/demo-reset-closeout.md`）——收审计刀 12 的 C-P1-3（演示库被探针污染：会话 122 里 33 条空会话、探针资产 16、pending 工单 18、`audit_log` 842），**无表无迁移**：新增 `scripts/demo_reset.py`（**默认 dry-run、`--apply` 才写库**；清**两类特征行**——空会话（无消息/工单/评分/缺口/回流锚）与探针资产（`mcp_registered` + `^(mcp-smoke|evidence probe)` → **置 discarded 不删行**）；知识缺口与工单**只报告不清**——那是缺口闭环/转人工闭环的演示素材）+ README「演示前检查」补清理一节 + 3 例真库测试（只清特征行/不误伤/幂等/无 `--apply` 不写库）。实测：dry-run 33/16 → `--apply` 后空会话 0、探针资产 0、会话 124→91，缺口与工单原样；接口复核探针资产 0 条。集成 957→960 passed。审计刀 12（PR #90）、第 60 刀 golden 维护（PR #89）、第 59 刀工作队列折叠（PR #88）、第 58 刀弱命中收口（PR #87）、第 57 刀会话详情（PR #86）均已合并 main。
