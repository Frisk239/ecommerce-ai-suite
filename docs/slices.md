...
上一刀：**第 87 刀 faithfulness judge 跑全**（`feat/faithful-judge-87`，closeout 见 `docs/progress/faithful-judge-87-closeout.md`）——goal §6.2 最后一条遗留收官：runner judge 列加韧性（单条 3 次重试 + fail-soft；**中断史根因=单条 LLMError 即永久终止整列**，非网关问题；线上 llm 20s/0 契约不动）；judge 首次跑全 96 条/answered 80 **全部评上零失败**（工件 `scripts/eval/out/report-judge-87.md`，检索层六列与 83 刀基线逐位相同）；**定位订正：overall 忠实度 10.0% 不是幻觉率**——严格逐句原文对照口径下模板组装框架语的措辞越界（对照实验 orig False/stripped True 实锤；判 supported 的 8 条全为 dialogue 资产=原文粘贴形态，判分面按资产种类二分；两遍复跑聚合逐位一致）；goal.md §6.2 未完成清单清零（Wikidata 空 schema→74 刀、judge 列→本刀）；顺手清 84 刀记债：`fetch_published_refs` 补 `discarded_at` 过滤（运维手工 SQL 废弃形态，集成钉子回退验证变红）；集成 **1204 passed/0 skipped**。第 86 刀（PR #122）——OOV 对照商品表两类分说：**在库资料缺**（「资料还在补充中」+落缺口+工单）/**不在库**（「本店暂时没有 X」+不落缺口+工单——缺口池不再被经营范围问题污染）；`oov_product_match` 双向判据（实体⊂库内名任意长；库内名⊂实体要求 ≥3 字且覆盖率 ≥0.6）+ 类目/别名按 `target in universe` 过滤；存量 8 条 OOV 悬空缺口外科收口；集成 1200 passed。第 85 刀（PR #121）——OOV 判定性能：语料快照缓存（键=库身份+资产摘要 sha1，发布/改标题/废弃即失效），oov_verdict 11.8ms→3.2ms、每 ask 990→524 行，行为零变化；集成 1197 passed。第 84 刀（PR #120）——结转记债三件：`_can_publish` 废弃闸（评审追加 retry_machine_wash/_can_replace_version_bytes/asset_view 三处同族）、`service_session_transitions_total{from,to}` 三处埋点、OOV 拒答首行点名实体（过 redact）；集成 1196 passed。第 83 刀（`feat/data-hygiene-83`，closeout 见 `docs/progress/data-hygiene-83-closeout.md`）——走查起最后一条挂起裁决项收口：核实推翻「探针资产」判断（260-262 实为**缺标题的真实资料**、269 是 268 的劣化副本）→ 补 3 标题/废弃 2 重复；**顺带揪出真缺陷**：retrieve/oov 三处候选 SQL 漏过滤 `discarded_at`（0042 废弃语义在证据面没落地，本刀首次出现「已发布后废弃」才暴露）→ 修 3 处 + MCP 导出/取全文 2 处（评审 P1）+ 钉子；评测认账变动（positive@1 87.5→90.0 / paraphrase@1 92.0→88.0 / overall 持平，syn-005 位移经 Spec 轴订正为横移噪音非「更准」）；集成 1191 passed/0 skipped。第 82 刀（`feat/oov-gate-82`）——Owner 裁决「实体存在性闸」：问句点名库内不存在的实体时不再引他品证据（症状：问「雀巢咖啡」答花生酱的资料）。判据三轮迭代定稿（剔观点尾+字段词表 → ≥4 字连续零出现串 + 实体段亲和 0；golden 96 条零误杀，命中 10 条全为 refusal 组）；引擎 RAG 分支收口为拒答（缺口/工单复用既有出口，`fallback_reason=oov`）；小语料护栏 100 条（评审回退实验实测误杀 4 例）。live：三条 OOV 问句全拒答（缺口 G-0065+ 工单），对照零回归。集成 1183→1188 passed/0 skipped。评审 P1×3 实修（假钉子换真承重形态 ×2（亲和/span 各一双向断言）+ 全出口裁决 + Must「进引擎金标」偏差如实记录）。第 81 刀（`feat/coverage-gate-81`）——Owner 裁决「覆盖闸误拒治本」：测量先行，**intake 主假设（覆盖度被商品名稀释）被证伪**（coverage 0.429 且 hits=5，闸不触发）；真因=**prompt 证据行只给 `[来源：A-226·v1]` 无资料名** → 模型无法核验实体 → 自述未覆盖 → no_coverage 收成拒答（降级模板一直带标题，只有 LLM 路径缺）。修法：证据行补资产标题（过 `redact_contact` + 结构净化）+ 系统提示教实体核验 + 问句行同步强掩。live：**3/6 拒 → 5/5 答**（两轴独立复验），压力场景无张冠李戴错误值（M&M black→拒答、保温壶→明确说对象不符）；集成 1180→1183 passed/0 skipped、评测逐位不变。审计刀 16（`feat/audit-16`）——第 76–80 刀三轴审计：P0×2 实修（裸覆盖标记「[未覆盖]」退化答案句级 fullmatch 收口+5 钉；具名 OFF 问句不稳定拒答=存量数据错配，README 避开清单+记债）、P1×4（**并发回流孤儿资产按 0042 废弃补偿收口+钉子**、ADR 0023 幻影 closed 态订正、roadmap CSAT 承诺回写、OOV 残差记债）、P2 若干全处置；三轴 live 复核 79 rerank 顾客面 8 问/80 结束会话全路径/76-78 抽查全部通过；演示库外科清 7 空会话+2 孤儿资产；集成 1174→1180 passed/0 skipped、评测逐位不变（87.5/86.2）。第 80 刀（PR #115）——Owner 裁决「允许给顾客加结束会话选项」，48 刀 CSAT 回退点解冻：`POST /api/customer/sessions/{id}/end`（幂等；ended 终态 + closed_at 落 DB 钟）；闸语义「结束=关对话流不关善后」（ended 下发问 409、评分/反馈/联系方式放行——**结束后评分闭环兑现**）；ended 可回流登记（closed_at 保持顾客结束时刻）；**评审 P1 实修并发 TOCTOU**（end/回流收口改原子条件更新，旧读-判-写在 register_asset 机洗 commit 窗口下被探针实测穿透）；顾客页结束按钮+「会话已结束」横幅+composer 锁+评分条保留，客服页「已结束」徽章；浏览器全路径验收（结束→5 星→新会话恢复）；集成 1174 passed/0 skipped。**下一刀=审计刀 16**（覆盖第 76–80 刀）；仍待裁决：已发布探针资产 260-262/268 处置。第 76 刀（PR #111）/第 77 刀（PR #112）/第 78 刀（PR #113）/第 79 刀（PR #114，实体亲和重排，检索基线刷新 87.5/86.2）/第 80 刀（PR #115）均已合并 main。——Owner 裁决「跨商品混淆必须解决」：75 刀留下的对照基线兑现为 **ADR 0048 实体亲和重排**（问句对候选资产标题的 IDF 加权 bigram 覆盖 ×3 per-query 乘数，纯词法零 LLM 零向量；实验矩阵定稿 α=3，`scripts/eval/rerank_experiment.py` 入库）。**检索基线刷新 87.5/86.2**（positive@1 72.5→87.5、overall@1 66.2→86.2、混淆@1 60.0→73.3；拒答/误拒零回归；top-1 变动 18 改善/2 变差/3 横移（脚本口径），变差两条均为语义歧义非硬伤）。live：75 刀两病例修复（净含量引 A-0009 正解、材质引 A-0003 进窗）。集成 1166 passed。第 78 刀（PR #113）已合并。仍待裁决：已发布探针资产 260-262/268 处置。第 78 刀（PR #113，纯文档刀：README 演示路径节+SO-1001 口播）已合并——走查方案三刀（76/77/78）此前已全部交付。

当前阶段：goal §6.2 面试级八条**无在案遗留**（第 87 刀收官最后一条 faithfulness judge 列）。**下一刀按节奏=审计刀 17（覆盖第 81–87 刀）**。余下大块待 Owner 定：部署面（反代/HTTPS/备份/密钥生产化/压测——此前暂缓）、发布冲突检测（多源治理最后一块，审计刀 10 有真实病例：两份已发布退货政策打架）、OOV 中间地带保守度（语义裁决）。
...
## 第 72 刀：演示素材修复——「你们几点上班」补口径闭环（已交付，`feat/hours-gap-72`）

**路径：** 审计刀 13 C 轴：G4「你们几点上班」曾被 asset 17《补口径 · 你们几点上班》v1 发布解决，但 v1 正文**误植为会员积分内容**——`retrieve("你们几点上班")` 0 命中，同问再问仍拒答开 G5，「拒答留缺口→去补→再问命中」治理主线在这一对上演不出来。本刀**纯演示数据**（治理流程操作，仓库侧仅文档）：在 asset 17 上**开修订→换正文（客服上班时间：每天 9:00–21:00 营业，21:00 后留言次日 9:00 回复；节假日不休息）→发布 v2**——正是「去补口径」该有的样子，顺带修正误植。**live 实测**：顾客问「你们几点上班」→ **answer 引 asset 17·v2**，正文「客服上班时间为每天 9:00–21:00 营业…」；**G5 被答案自动收口**（审计刀 11「答上即关」），G4/G5 双 resolved——治理闭环整条可演。诚实披露：v1 的积分内容被 v2 整体替换且库里无其它承载（重问积分会再落缺口走治理——流程的正确形态）；写副作用=修订+发布+1 验收会话+G5 收口；无代码变更（1158 基线不动）。证据见 `docs/progress/hours-gap-{intake,closeout}.md`。
...
## 审计刀 15：三路并行只读审计第 71–75 刀（已交付，`feat/audit-15`）

**路径：** 三轴（A 设计 / B 对抗 / C live）。**P0×0 三轴全零**。**P1×2（全实修）**：①**eval runner 拒跑**——B 轴实测 `run_eval.py` exit 2：72 刀把 asset 17 v1→v2 发布后 golden 的 pos-038 期望过期（60 刀加的 fail-closed 守卫**正确拦截**，但 72/74 两把演示数据刀都没复跑 eval——「发布类治理动作后核对 golden」被静默违反两刀，CI 不跑 eval 所以断链是静默的）。处置：pos-038 期望 17/v1→17/v2（top-1 仍是 asset 17）；ref-011「你们营业到几点」被 72 刀的上班时间文档**合法答掉**——按生成协议（有对应资产的应拒答候选剔除）换同域零命中话题「可以开发票吗」（逐词法验证 0 命中）。**新基线：positive 72.5/77.5、overall 66.2/76.2、拒答 100%、误拒 0.0**——比旧基线 70.0/65.0/2.5 **真改善**（74 刀发布的 Vivo 规格资产带来：一条 positive 新命中 + 一条原误拒转正），检索本体零变化（75 刀结论仍立）；rag-eval-report 已加「审计刀 15 订正」节。②**陈旧缺口**：C 轴判定 8 条 open 缺口检索已可命中——G37/G38 用产品机制关闭（再问即答即收口）；G29 实问被覆盖闸**诚实拒答**（留治理，只读预测与 live 覆盖闸差异如实记录）；G23/G33/G2/G25/G30 留 open（后两者的 top 命中是无关评论——跨商品混淆病根实例，记档）。**P2 择要**：评分路径改**原子 UPSERT**（ON CONFLICT DO UPDATE——并发首评败者按唯一约束转入改评，不再吐语义过时的 409；live 复验）；csat 指标 docstring 订正（**事件计数**含改评，与仪表分布不同源）；73 刀清理（last_tool_subject 死参数、报价回落重复调用）；三处文档订正（slices 71 条目 1158、CONTEXT 评分可改移出待裁决、71 closeout 补验收副作用）。留档：asset 268 重复引用/材质 316 vs 钛钢/260-262 探针残留/评分刷新丢本地态。环境注记：CREATE DATABASE TEMPLATE 会崩 pgvector 容器——克隆走 pg_dump。集成 **1161 passed**；eval 恢复绿（**新基线 72.5/66.2**）。写副作用全清单见 closeout。证据见 `docs/progress/audit-15-closeout.md` 与 rag-eval-report 订正节。


## 第 76 刀：走查方案第一批（已交付，`feat/walkthrough-fixes-76`）

**路径：** `docs/walkthrough-gap-plan.md` §二/§三 的 P1-1、P2-1、P2-3 + 用户现场审计新增的资产列表标题溢出。**纯前端刀（无后端/迁移）**：①**考核题库行可达性**——行补 `role="button"`+`tabIndex={0}`+`aria-label`+Enter/空格键处理，第 4 列空 caret 改为**显式「开始作答」按钮**（可发现性 + 键盘双修）；实测对该行按 **Enter 打开「销售考核作答」抽屉** ✓。②**计数首帧**——资产页 tab 与客服页会话筛选在 `state.phase !== 'ok'` 时显「—」（不再先闪 0）。③**考核中文优先**——题库按含 CJK 优先稳定排序（首行「保温杯的净含量是多少？」✓，英文题沉底保序）。④**标题溢出（用户审计）**——根因：`.table-gov td { white-space: nowrap }` 而标题格只有 `max-w-[22rem]` **漏了 truncate**（同表「失败原因」列早有该口径），长标题溢出单元格压到「种类/来源/状态」列（用户截图实证）；修法：资产主行/折叠行/缺口原问/素材中心/考核题面**五处统一补 `truncate`** + hover 全称 `title`——截图复验长标题带省略号、列间干净，DOM 量测考核页 6 格 overflow=0。前端 lint 7/0、build 绿；后端零改动（集成 1161 基线不动）。诚实披露：首帧「—」只有代码级验证（本地加载太快抓不到 loading 帧）；极长标题会把行尾 stale 徽章挤出可见区（罕见取舍）；嵌套可点（行 + 行内按钮）行为一致。证据见 `docs/progress/walkthrough-fixes-2-{intake,closeout}.md`。


## 第 77 刀：会话搜索 + 评分「改过」可见（已交付，`feat/session-search-77`）

**路径：** 走查方案 P2-2（120 个会话只有两个筛选，找演示会话只能翻）+ P2-4 操作者半项（★ chip 看不到「改过」）。①**会话搜索**——ServicePage 搜索框（首问包含、大小写不敏感 / 会话号精确），`?q=` 与 `?session=` 深链互不抹除（函数式 setSearchParams + replace），「N 条匹配 · 清空恢复」提示，搜索**不改**工单置顶排序；**走查中自抓自修**：首版号比较只认裸数字、`#204` 搜不到（占位符自己写着 #125）——浏览器验收当场抓到，修为 `#` 前缀剥离。②**改过可见**——`SessionDetail` 补 `rating_updated_at`（None=首评未改），详情 ★ chip 对改评过的会话显示「★ 3 · 改过」（悬停注明第 71 刀可改）。实测：搜「笔记本」**10 条匹配**；`?q=%23204` 深链 **1 条**；点开 #204（第 71 刀走查时 4→5→3 三连改的会话）详情 **「★ 3 · 改过」** ✓。集成 **1161 passed**（detail 测试扩三断言：首评 None/改评非空/详情读最新）；前端 lint 7/0、build 绿。披露：顾客面「刷新回显」不做——顾客令牌不跨刷新（第 45 刀口径），GET 自评没有承载面（记为依赖会话持久化的未来裁决）；搜索客户端过滤，120 条量级够用。证据见 `docs/progress/session-search-{intake,closeout}.md`。


## 第 78 刀：演示路径 + SO-1001 口播对齐（已交付，`docs/demo-path-78`）

**路径：** 走查方案 §五 第 3 条。**纯文档刀（README 两节）**：①SO-1001 口播对齐——演示库该单已被第 44 刀退货流真跑过、状态停在「退货中 · 3 个物流事件」，订单工具说明处加「口播注意」别念「已发货」（退货中本身就是「退货确认迁移订单状态」的演示素材）；②README「演示路径」节——**主线素材七条**（#199 治理闭环：拒答留缺口→补口径发布 v2→再问命中；#200/#202 多轮工具追问；#203 治理写回可追溯（Vivo 品牌 vivo ← A-0269·v1）；#125 类目价+拒答对比；#148 澄清闭环；#198 评分可改（★ chip「· 改过」）；#142 拼接追问），每条注明演什么对应哪刀，**全部是本会话真跑过的会话**（取自审计刀 13/14/15 C 轴实测注记）；避开清单（旧形态会话会误导）+ 避开方法（demo_reset --apply）；治理面说明（工单 29/缺口 20 不清——素材，用 #125 的工单演示转人工闭环即可）。披露：会话号属当前演示库，整库重灌后号会变（节首已写可按首问搜索）。证据见 `docs/progress/demo-path-{intake,closeout}.md`。

## 第 79 刀：实体亲和重排（已交付，`feat/rerank-79`）

**路径：** Owner 裁决「跨商品混淆必须解决」→ rerank 立项（75 刀实验数字即对照基线，停建清单「没有评测数字不许上」已满足）。**无新表无迁移**（候选 SQL 随带 `assets.title`）。病根：字段块（品牌：/净含量：…）跨资产同文/近同文，字段名 bigram 分数并列、短块微弱胜出、`(asset_id, chunk)` 稳定出榜——75 刀已证伪全局来源权重（不随问句变，被词法大分差吞掉）。本刀补 **per-query 实体信号**：`title_affinity` = 问句 terms 对候选资产标题的 IDF 加权 bigram 覆盖（共享 bigram 天然低 idf、专名天然高 idf），打分后乘 `1+3·aff`（ADR 0048；`AFFINITY_ALPHA=3.0` 实验矩阵定稿——α=4 时 confusion 回落）。落点在 `retrieve()` 内=引擎双通道/MCP/缺口验证/评测四出口同语义；问句与标题无 bigram 交集时乘数恒 1（中性面——标题含字段词/回流首问时纯字段问也有亲和，ADR 已知取舍）。**新基线 87.5/86.2**（+15.0/+20.0pp，混淆@1 +13.3pp，拒答/误拒零回归）。实验脚本 `scripts/eval/rerank_experiment.py` 入库且复算与生产 after 逐位一致。live（客服页 #240）：「保温杯的净含量是多少？」引 A-0009（75 刀 top3 全 OFF 他品）；「保温杯的材质是什么？」引 A-0003 进窗（75 刀 top1 被探针对话抢走）。集成 1161→1166 passed。证据见 `docs/progress/rerank-79-{intake,closeout}.md` 与 `docs/research/rag-eval-report.md` 第 79 刀节。

## 第 80 刀：顾客侧结束会话 + 会话终态语义（已交付，`feat/session-end-80`）

**路径：** Owner 裁决「允许给顾客加结束会话选项」→ 48 刀 CSAT 回退点（「本仓顾客通道没有顾客侧结束事件，等它出现等于永远不出现」）解冻。**无新表无迁移**。状态机三态：active→ended（顾客结束，幂等，closed_at=func.now()）/ active→registered（现状）/ ended→registered 放行（closed_at 保持顾客结束时刻——终结时刻以首次为准）。闸语义「**结束=关对话流不关善后**」：ended 下发问 409（`_status_label` 中文映射）、评分/反馈/联系方式放行；registered 全部照旧 409。顾客页：顶栏「结束会话」次级按钮 + 「会话已结束」横幅 + composer 锁 + 评分条保留（结束后评分闭环）+「开始新会话」；客服页「已结束」徽章 + ended 横幅按 canRegister 分支 + 回流按钮文案 ended 下切「回流登记」。**两轴评审**：Standards P1（并发 TOCTOU——end/回流收口旧读-判-写形态在 register_asset 内部 commit 的机洗窗口可达 20s 下被探针实测穿透：closed_at 被重写/ended 半态）实修为原子条件更新（WHERE status='active' / WHERE status IN (active,ended) AND registered_asset_id IS NULL + COALESCE）；P2×10（409 文案裸值、docstring 与行为相反、过期态结束按钮、README/CONTEXT 漂移、回流 Python 钟统一 DB 钟）全实修。集成 1166→**1174 passed/0 skipped**（+8）。浏览器全路径：结束→评分 5 星→新会话恢复。证据见 `docs/progress/session-end-80-{intake,closeout}.md`。

## 审计刀 16（feat/audit-16）——第 76–80 刀三路审计

2026-09-11。A 设计 0/2/5、B 健壮 0/1/6、C 产品+live 2/3/4，全数实修/处置。要点：①**P0 裸覆盖标记漏出**（「[未覆盖]」带引用作答 ~8%）——`_BARE_COVERAGE_RE` 句级 fullmatch 收口（不误伤事实否定句）；②**P0 具名 OFF 问句时答时拒**=50 刀导入数据标题/正文品牌错配的存量暴露——README 避开清单+记债；③**P1 并发回流孤儿资产**（B/C live 复现 201+409 双资产+孤儿字节）——409 分支按 0042 废弃语义补偿收口+monkeypatch 竞争钉子，演示库 2 份探针孤儿同口径手工废弃；④P1 文档批（ADR 0023 幻影态/roadmap CSAT 回写）+P2（register docstring 自相矛盾=孤儿认知根源、操作者 409 中文化 STATUS_LABELS 单一来源、slices 头部层叠收敛、README 计数漂移）。记债：title 命中下 no_coverage 误拒、OOV 拒答分支、会话生命周期指标、亲和缓存（规模触发）。集成 1180 passed/0 skipped；评测逐位不变。
## 第 81 刀：证据行补资料名（已交付，`feat/coverage-gate-81`）

**路径：** Owner 裁决「覆盖闸误拒治本」。症状「M&M white的条码是多少」6 次 3 拒。**测量先行**：intake 假设（覆盖度被商品名稀释 → 阈值误触发）**证伪**——coverage 0.429 但 hits=5，覆盖闸两条件都不成立；真因是**证据行缺归属**：块「条码：000000000063」不含商品名，模型无法确认归属 → 忠实性约束下自述「证据未覆盖」→ no_coverage 收口（降级模板路径一直带标题，仅 LLM 路径缺）。修法（信息补齐，非调松闸）：`build_prompts` 证据行 -> `[来源：A-226「M&M white 规格（OFF）」·v1]` + 系统提示教实体核验 + 标题结构净化（换行/标签字符，title=顾客可影响面）+ 问句行升强掩；chat_engine meta 复用。live 3/6 拒 → **5/5 答**；压力场景（库中不存在的相似商品）零张冠李戴错误值。集成 1183 passed/0 skipped、评测逐位不变。记债：中间地带（修饰语不可证伪）、一次保守误述、golden_engine 未补的理由（LLM 行为面不进确定性层，替代=prompt 契约单测 + vendor 引擎级 wiring 断言）。
## 第 82 刀：实体存在性闸（已交付，`feat/oov-gate-82`）

**路径：** Owner 裁决。症状=问库内不存在的商品（雀巢咖啡）引他品证据（花生酱）。判据：剔观点尾与库内字段名词后，实体段存在 ≥4 字连续零出现串且最高标题亲和为 0 → OOV。三轮测量迭代（第 1 轮误杀 syn 改写 3 条、第 2 轮单字维度分不开、第 3 轮定稿）；golden 96 条零误杀。落点=引擎 RAG 分支拒答收口（缺口/工单/文案复用；`fallback_reason=oov`）；`OOV_MIN_CORPUS_ROWS=100` 小语料护栏。live 三条 OOV 全拒答、对照零回归。评审 P1×2：假钉子（E4/E5 打不红）换成真承重形态（多实体问句，回退复验必红）+ 全出口同语义 Must 的裁决（OOV 是作答决策，不下推 MCP）。集成 1188 passed/0 skipped。
## 第 123 刀：拉丁整词 token + 规格名头行 + 字段行定向重排（已交付，`feat/latin-word-token-123`）

**路径：** 生产级数据重灌（OFF/Wikidata/Commons/ABCD/Dell）后七站重验，第一站即翻车——「Nutella的净含量是多少」检索有命中却拒答，剥出三层病根全修根。①**检索权重**：统一滑窗 bigram 把拉丁词切 O(len) 单元（Nutella=6 个 vs 净含量=2 个），品牌块恒压数值块、证据窗被标题块占满→模型如实自述未覆盖→58 刀收口拒答；改 `_token_spans` 原始串扫描：ASCII 字母/数字段整词一 token（小写归一修大小写洞；字母↔数字边界切分；空格逗号天然断词），CJK 逐 token 等价，索引零重建；OOV 零出现串改 token 字符位扫（CJK 逐位等价、拉丁整段计长、实体串保留原大小写）。②**数据断层**：Wikidata 规格正文只有翻译值（品牌：森海塞尔）与拉丁商品名零词法交集——旧评测「正确」实为 800↔2009 共享 bigram「00」的假阳性；`spec_text` 补「{name} 规格」头行（OFF 先例）。③**证据窗收口**：数值行词法分天然低（长值撑大分母），名头行/图片描述/兄弟头行占满前 2；打分乘数方案（×10）实测把兄弟商品短字段块抬到第 1（-4.4pp 三条全坏）被否决，定稿**零分值保序重排**（`_field_directed`：问句点名字段时 top-1 资产自己的该字段块提到最前，跨资产次序与分数不动）。评测（生产 golden 89 条）：88.4→84.1，-4.3pp 逐条定位=两条同商品跨资产引用翻转（生成器从图片资产造题、规格资产同商品微弱胜出）+一条纯噪声彩票，真实问句零回归。E2E 六问全对（Nutella 400g/Sennheiser 2009 年/Coca 配料表/保温杯钛钢/森海塞尔类目/星巴克 OOV 拒答+工单），前三条本刀前全拒答。重灌链自包含（wikidata 步骤并入+OFF 资产级幂等+对话步骤 --register 用法修正）。集成 **1709 passed**。证据见 `docs/progress/latin-token-123-closeout.md`。

## 第 124/125 刀：切片真相链修根 + 真实直播素材接入（已交付，`feat/truth-chain-124-125`）

**路径：** Owner 裁决「验收问题落路线+直播素材调研抓取+洗帧修复+健壮性审计」→ `docs/roadmap-production-hardening.md` 立项（124-126）。**124 真相链修根**：已发布视频资产文案说保温杯、画面是显示器（生产重灌实证）——根因三层：`register_recording` 顺手默认动作劫持跨场候选（46 刀单录像时代裁决）+ reseed `pending[:3]` 全局盲选 + 洗帧商品匹配无声跳过。修法：自动绑收窄为同名批次（`source_video_label == label`；ASR 候选创建即绑定不受影响，异名走 49 刀显式改绑）；reseed 视频链重构 `_video_chain()`（转写挂类目商品锚/按录像拣选逐条打印/失败必有声）；错资产不可废弃（0042 闸门）→ 修好链路清库重灌纠错。验证：种子候选 recording_id 全空（不再被劫持）、Dell 拣真英文转写、**保温杯材质从「带引用作答」变诚实拒答+缺口 G-1**（旧「正确」一直引污染转写）。**125 直播素材**：调研定案 Samsung「Unbox & Discover 2023」官方发布会直播（**CC BY 3.0**、40 分钟多商品章节、184MB 直链；学术数据集协议墙高，见 `docs/research/live-stream-sources.md`）；接入重灌链实测：40 分钟→**60 条 ASR 候选**、4 条发布、A-40/41 洗帧出分（score 8，VLM 评注「电视主体+主播头像」三一致）、帧资产 A-42 cut_from A-40·v1；E2E 六问含 **Dell 视频问句引 A-34/36 答出型号**、Unboxed and Discover 引 A-38。集成+单测 **1703 passed**（junitxml 口径，集成环境 0 skip）。证据见 `docs/progress/truth-chain-124-125-closeout.md`。

## 第 126 刀：健壮性审计（静默失败同族三路，P0/P1 零）

**路径：** 124 刀实证的「静默跳过」族系统扫描——脚本面（17 文件 except 静默判据）+服务面（18 文件）+端到端（重灌日志 vs 库内真相对账）。**P0×0 P1×0 P2×2 顺手实修**（video_compose 图片尺寸降级 (0,0) 无声→logger；coach_roleplay 整改建议解析失败置空无声→logger）；P3×3 留档（asr 段级丢失有全局兜底、demo_reset 孤儿字节计数、Wikidata 概念条目混入商品表）。测试顺手件：affinity 自检钉子隐含干净语料假设（集成环境整模块跑插队打红，纯净 main 复现）→ 加固为两资产配对断言。端到端对账零声明-真相偏差。证据见 `docs/progress/robustness-audit-126.md`。
