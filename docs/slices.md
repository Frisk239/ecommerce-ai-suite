...
上一刀：**第 17 刀素材中心**（`feat/material-center`，closeout 见 `docs/progress/material-center-closeout.md`）：任务五态机（ADR 0038）+ 规则质检/抽检过线才登记（0029）+ material_generated 来源兑现 + 机洗打码步（audit-3 P1#4：转写送厂商前+抽出值打码，版本字节不动）+ 闸门 kind 条件修复（「素材没有规格必填」词条兑现）。能力块 4.5/7。第 11–16 刀+审计刀 3 已合并 main。

当前阶段：工程刀。下一刀：**第 18 刀直播切片**（候选→拣选→登记 0014/0015）；审计刀 4 于第 20 刀后触发。
...
| 刀 | 约束 |
| --- | --- |
| 第 18 刀直播切片 | 切片候选（不是资产 0014）→拣选登记（0015 视频资产）+源录像词条+切片汇入视图 |
| 第 19–20 刀 | 销售考核 → 血缘视图（0026/0027 已锁约束） |
| **审计刀 4** | **第 20 刀后触发**（候看：17 刀评审债务七条、素材打回路径 UX） |
