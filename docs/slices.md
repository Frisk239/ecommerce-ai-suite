...
上一刀：**第 55 刀 四份数据集在产品面逐个可见**（`feat/dataset-labels-55`，closeout 见 `docs/progress/dataset-labels-closeout.md`）——修审计刀 10 P1-3（Wikidata/OpenFoodFacts/WANDS 原来同叫「开放数据集」分不出哪一份），**迁移 0028**：`SOURCE_KINDS` += `wikidata`/`openfoodfacts`/`wands`（`open_dataset` 保留为通用类、库里不再使用）；迁移按与 0026 同源的形态把 `open_dataset` 拆到具体数据集（资产 `%规格（OFF）`→openfoodfacts；商品 WANDS 承载→wands、六类目空模板→wikidata、食品单字段→openfoodfacts，只动 `open_dataset`、幂等、down 原样收回）；三个导入脚本写各自词；README 来源表改逐数据集。实测：商品 `wikidata 91 / openfoodfacts 20 / seed 2 / wands 1`、资产 `openfoodfacts 20`；商品页出现三个新词且不再出现「开放数据集」。集成 930→934 passed。第 54 刀 widget 宿主站点（PR #82）、第 53 刀 CSAT 留言可回溯（PR #81）、第 52 刀类目问价（PR #80）、审计刀 10（PR #79）、第 49–51 刀（PR #76/#77/#78）均已合并 main。
