...
上一刀：**第 59 刀 工作队列导入货折叠**（`feat/workqueue-fold-59`，closeout 见 `docs/progress/workqueue-fold-closeout.md`）——收审计刀 8 起记债的「工作队列首屏被导入货淹掉」（审计刀 9/10/11 三次复核仍是 194 行里 180 条导入），**无新表无迁移、纯前端**：`workQueue.isImportedSource`（按来源词判「数据集导入」）在表格末尾插一行折叠头「数据集导入 N 条（评论语料/开放数据集，非人工登记）」，默认收起、展开后与主表同列同序；**默认 tab/范围、总数与次序都不变**，任何一条都还能看到。实测：默认 **15 行**（14 条人工 + 1 折叠头）↔ 展开 **195 行**。集成 945 passed（后端未改）；前端 build 绿、lint 7/0。第 58 刀弱命中按拒答收口（PR #87）、第 57 刀会话详情补齐站点/访客/评分（PR #86）、第 56 刀类目别名（PR #85）、审计刀 11（PR #84）均已合并 main。
