...
上一刀：**第 47 刀 可观测最小版**（`feat/observability-47`，closeout 见 `docs/progress/observability-closeout.md`）——roadmap v3 第二梯队第三项（**无新表无迁移**，新依赖 2 个）：structlog JSON 日志（既有 `getLogger` 调用一行未改即变 JSON）+ 纯 ASGI correlation-id 中间件（`X-Request-Id` 回显、contextvar 进日志）+ instrumentator HTTP RED + 三个自定义（`chat_requests_total{channel,kind,generated}` / `ttft_seconds` / `llm_tokens_total{direction,model}`）+ `METRICS_TOKEN` 闸的 `/metrics`（空=401 fail-closed）+ 可选 prometheus profile。钉测 24 例；真容器端到端：顾客真问一句 → `chat_requests_total{channel="customer",kind="answer",generated="true"} 1`、`ttft_seconds_sum 8.38`、`llm_tokens_total` 输入 455/输出 331（厂商真 usage）、同一请求链（httpx→网关→access）共用一个 correlation_id。集成 816→840 passed。第 46 刀切片真链路（ADR 0047 + 迁移 0023）、第 45/44/43/42/41 刀与审计刀 6/7/8 均已合并 main。

当前阶段：**第三阶段产品硬ening**（施工权威 `docs/roadmap-product-hardening.md`）。**审计刀 8 已完成**（三路并行审计 **P0 全零**，P1×5 实修；证据 `docs/progress/audit-8-closeout.md`）。**第 46、47 刀已交付**（切片真链路 ADR 0047 + 迁移 0023；可观测最小版）。下一刀：**第 48 刀 CSAT + 反馈闭环补全**（会话结束 1–5 星、thumbs-up 补收、打回补理由、43 刀仪表加 CSAT 卡）；第 48 刀后按节奏 **审计刀 9**（第 50 刀之后仍有第三梯队按需裁决）。**待 Owner 裁决**：审计刀 8 记债的四条产品面缺口（商品价 3/115、多来源在产品面不可见、工作队列首屏 183 条原始灌入、widget 不落宿主 origin）。
