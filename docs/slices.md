...
上一刀：**第 69 刀 评论闸观点尾白名单扩表**（`feat/opinion-tail-69`，closeout 见 `docs/progress/opinion-tail-closeout.md`）——收复审审计记的固有边界（形容词+吗 体感问错杀）。**无表无迁移**，**设计经评审证伪后反转**：初版用「事实问尾白名单」收窄，评审实测「退货运费谁承担」「查物流」等未枚举同义事实问全部漏放并引无关评论（66 刀 P0 同义复现，错误答案）；终版**保持 66 刀默认拦**、把错杀改由**扩观点尾白名单**解决（`_OPINION_RE` += 好吗/结实吗/及时吗/顺利吗/麻烦吗/给力吗… 品质形容词+吗 家族）——漏枚举观点尾=诚实拒答（便宜），漏枚举事实尾=错误答案（贵）。实测：「客服态度好吗」「快递包装结实吗」「发货及时吗」恢复评论证据；坏 case 全不变（到货了吗无命中、退货运费无论「多少钱/谁承担/要运费吗」都首引政策、查物流诚实拒答）。live：客服态度好吗→引评论作答。真值表改判/新增 14 行、引擎金标 +1；集成 1093→1108 passed；检索金标 96 例逐例零 diff。第 68 刀（PR #100）已合并 main。**下一刀=第 70 刀 无单号订单问句归宿**，其后审计刀 14。
