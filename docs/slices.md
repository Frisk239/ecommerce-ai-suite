...
上一刀：**审计刀 11**（`feat/audit-11`，closeout 见 `docs/progress/audit-11-closeout.md`）——三路并行只读审计第 51–55 刀（迁移 0027–0028）：**P0×2 实修**（①会话详情恒回 `host_origin: null`（列表有值、详情没传——三轴同时命中）②治理台「待补缺口」里躺着客服已能答的问题：`run_ask` 的目录回落分支现在把同问 open 缺口一并收掉 + 演示库两条陈旧缺口手工关闭）+ **P1×5 实修**（类目问价被「商品名含类目名」劫持 → 改先类目后单品／导入脚本补写资产来源（重置库不再丢来源词）／迁移用例第三次踩「升到 head」→ 钉到 0028 并加无残留断言／商品页首屏 115 卡（全店有价后折叠判据失效）→ 按数量收口 24 张 + 折叠／两处披露订正（器皿区间其实可见、分布漏 1 条手建））+ P2 五条（host_origin 截断、类目混币种拒答、轨迹价格口径、四处 open_dataset 旧注释、CONTEXT 词条补到 11 值）。集成 934→937 passed。第 55 刀数据集逐个可见（PR #83）、第 54 刀 widget 宿主站点（PR #82）、第 53 刀 CSAT 留言可回溯（PR #81）、第 52 刀类目问价（PR #80）、第 51 刀工作队列筛选（PR #78）、审计刀 10（PR #79）均已合并 main。
