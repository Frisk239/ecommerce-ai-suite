...
上一刀：**第 80 刀 顾客侧结束会话 + 会话终态语义**（`feat/session-end-80`，closeout 见 `docs/progress/session-end-80-closeout.md`）——Owner 裁决「允许给顾客加结束会话选项」，48 刀 CSAT 回退点解冻：`POST /api/customer/sessions/{id}/end`（幂等；ended 终态 + closed_at 落 DB 钟）；闸语义「结束=关对话流不关善后」（ended 下发问 409、评分/反馈/联系方式放行——**结束后评分闭环兑现**）；ended 可回流登记（closed_at 保持顾客结束时刻）；**评审 P1 实修并发 TOCTOU**（end/回流收口改原子条件更新，旧读-判-写在 register_asset 机洗 commit 窗口下被探针实测穿透）；顾客页结束按钮+「会话已结束」横幅+composer 锁+评分条保留，客服页「已结束」徽章；浏览器全路径验收（结束→5 星→新会话恢复）；集成 1174 passed/0 skipped。**下一刀=审计刀 16**（覆盖第 76–80 刀）；仍待裁决：已发布探针资产 260-262/268 处置。第 79 刀（PR #114）已合并——实体亲和重排（ADR 0048）检索基线刷新 87.5/86.2。——Owner 裁决「跨商品混淆必须解决」：75 刀留下的对照基线兑现为 **ADR 0048 实体亲和重排**（问句对候选资产标题的 IDF 加权 bigram 覆盖 ×3 per-query 乘数，纯词法零 LLM 零向量；实验矩阵定稿 α=3，`scripts/eval/rerank_experiment.py` 入库）。**检索基线刷新 87.5/86.2**（positive@1 72.5→87.5、overall@1 66.2→86.2、混淆@1 60.0→73.3；拒答/误拒零回归；top-1 变动 18 改善/2 变差/3 横移（脚本口径），变差两条均为语义歧义非硬伤）。live：75 刀两病例修复（净含量引 A-0009 正解、材质引 A-0003 进窗）。集成 1166 passed。第 78 刀（PR #113）已合并。**下一刀=第 80 刀 顾客侧「结束会话」+ 会话终态语义**（Owner 已裁决允许，48 刀 CSAT 回退点解冻）；审计刀 16 于第 80 刀后；仍待裁决：已发布探针资产 260-262/268 处置。第 78 刀（PR #113，纯文档刀：README 演示路径节+SO-1001 口播）已合并——走查方案三刀（76/77/78）此前已全部交付。
