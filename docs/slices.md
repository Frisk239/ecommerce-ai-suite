...
上一刀：**第 76 刀 走查方案第一批（前端四项）**（`feat/walkthrough-fixes-76`，closeout 见 `docs/progress/walkthrough-fixes-2-closeout.md`）——依据 `docs/walkthrough-gap-plan.md` P1-1/P2-1/P2-3 + **用户现场审计新增的标题溢出**。**纯前端刀**：①考核题库行补 `role=button`+`tabIndex`+Enter/空格（实测按 Enter 打开「销售考核作答」抽屉✓）、第 4 列空 caret 改**显式「开始作答」按钮**；②资产页/客服页计数 loading 态显「—」不显 0；③考核题库**中文优先**稳定排序（首行=保温杯净含量✓）；④**标题溢出修复**（用户截图实证：`.table-gov td` nowrap + 标题格漏 truncate → 长标题压「种类/来源/状态」列）——资产主行/折叠行/缺口原问/素材中心/考核题面五处统一补 `truncate`+hover 全称，截图复验列间干净。前端 lint 7/0、build 绿；后端零改动（1161 基线不动）。审计刀 15（PR #109）已合并 main。**下一刀=第 77 刀 会话搜索**。
