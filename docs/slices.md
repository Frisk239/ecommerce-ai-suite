...
上一刀：**第 20 刀血缘视图**（`feat/lineage`，closeout 见 `docs/progress/lineage-closeout.md`）：ADR 0026 落地——`GET /assets/{id}/lineage` 固定 4 查询拼装（引用 JSONB containment 下推/写回含回滚与字段派生/考核）+详情页血缘面板；零新表零写路径；评审实修回滚写回与 fields 派生两个 P1。第 11–19 刀+审计刀 3 已合并 main。

当前阶段：**计数线到——下一刀=审计刀 4**（自审计刀 3 后五刀：16 还债/17 素材/18 切片/19 考核/20 血缘；三路子代理，不改产品代码）。
...
| 刀 | 约束 |
| --- | --- |
| **审计刀 4** | **下一刀（计数线到）**；候看：17/18/19/20 刀债务、CONTEXT 事故复盘、词表误伤真实分布、兜底题证据维、MCP 导出留痕、运营 Agent（最后一块能力） |
| 运营 Agent | 能力块 7/7 的最后一块；工具轨迹+失败重试+投放确认（goal）——审计后按缺口裁决形状 |
