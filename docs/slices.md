...
上一刀：**第 68 刀 引擎路径金标进 CI**（`feat/engine-golden-68`，closeout 见 `docs/progress/engine-golden-closeout.md`）——Owner 裁决「先把产品做到足够完善」按推荐顺序开工，评测尺加厚先行。**无表无迁移**：`evals/golden_engine.json` 23 例 × 十面（catalog/stock/order/评论闸/rag/多轮）+ `test_engine_eval.py` 直调 `run_ask` 断言**确定性契约**（kind/tool/cites/文案）——**进 CI**（检索金标是本地口径，这层是门禁）；每个面至少一枚历刀真实缺陷的对位 case（62 别名/13 花多少钱/65 路由层/67 还有么/66 评论闸/13 拼接 merge），此后同类回归 CI 即红。种子自含（商品三类带价带库存+两文档+一评论资产）。集成 1069→1093 passed；检索金标逐位相同。同批 Owner 裁决记录：评分可改=允许（后续刀实现）、分币种报价=维持拒答、默认视角=维持现状。复审审计（PR #99）、第 67 刀（PR #98）、第 66 刀（PR #97）均已合并 main。**下一刀=第 69 刀 评论闸触发收窄**。
