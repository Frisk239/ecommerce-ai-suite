...
上一刀：**审计刀 10**（`feat/audit-10`，closeout 见 `docs/progress/audit-10-closeout.md`）——三路并行只读审计第 49–51 刀（迁移 0025–0026）：**P0×2 实修**（①报价纯度闸只装在两条报价路径中的一条 → 空命中库上「家具送货安装怎么收费？」被答成「售价 899元」，闸收敛到共用 `_quote_answer`；②演示库两份已发布退货政策打架 → 顾客答「存在两个口径」，走治理路径把 asset 13 修订发布 v4 对齐 7 天口径）+ **P1×6 实修**（纯度闸只认全名子串→改剔命中片段 `lcs_fragment`，部分名问价不再静默挡回／ADR 0045 补记纯度闸并订正拦截归属／ADR 0047 订正「多源留 Out」／三个导入脚本补写 `source_kind`／来源筛选的「隐身过滤器」与空态文案／`?source=` 取值校验）+ P2 六条（评测集 `什么` 疑问词漏项、`CONTEXT` 自相矛盾与来源词条补值、候选卡逐卡显示源录像标签、rebind 测试顺序独立…）。集成 919→921 passed。第 51 刀工作队列来源筛选（PR #78）、第 50 刀多来源可见+演示价（PR #77）、第 49 刀源录像可改绑（PR #76）、审计刀 9（PR #75）、第 46–48 刀（PR #72/#73/#74）均已合并 main。
