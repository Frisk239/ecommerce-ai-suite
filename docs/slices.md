...
上一刀：**第 36 刀检索同义词接线**（`feat/synonym-wiring`，closeout 见 `docs/progress/synonym-wiring-closeout.md`）——goal §6.2.2 纪律首战：替换式 after 净负（正例-5pp）→ Owner 裁决改**并集扩展**→全分布非降（正例/混淆零漂移、同义组 @1 56→60/@3 64→72、overall 65.0/75.0）+闭包局限声明+审计 P1×5 全修。审计刀 6 及第 26–35 刀已合并 main。

当前阶段：**面试级**（goal §6.2）。下一刀：**第 37 刀客服真 loop**（模型提议工具+代码授权+max_steps+轨迹可断言；**必开 ADR 修订 0036/0037**）→ 38 MCP 证据 → 39 自进化仪表 → 40 忠实度/反馈/两阶段写 → 审计刀 7。
