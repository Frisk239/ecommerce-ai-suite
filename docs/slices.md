...
上一刀：**审计刀 15**（`feat/audit-15`，closeout 见 `docs/progress/audit-15-closeout.md`）——三轴审计第 71–75 刀：**P0×0（三轴全零）**；**P1×2 实修**：①eval runner 在当前演示库拒跑（72 刀发布 asset 17 v2 使 golden pos-038 过期、ref-011 被上班时间文档合法答掉——60 刀守卫正确拦但 72/74 都没复跑 eval）→ pos-038 指到 17/v2、ref-011 换零命中话题「可以开发票吗」，**新基线 positive 72.5/overall 66.2/误拒 0.0**（74 刀 Vivo 资产的真改善，检索零变化）；流程约束升级：发布类动作合并前必须复跑 run_eval。②陈旧缺口处置：G37/G38 再问即答收口、G29 实问被覆盖闸诚实拒答留治理、G25/G30（无关评论命中）记为混淆病根实例。P2：**评分改原子 UPSERT**（并发首评败者转改评不再 409）、csat 指标 docstring 订正（事件口径）、73 刀死参数/重复调用清理、slices/CONTEXT/71-closeout 三处文档订正。集成 1161 passed；eval 新基线 72.5/66.2。第 75 刀（PR #108）已合并 main。**下一刀=第 76 刀**（方向待 Owner：部署面（搁置中）/rerank 观察项/演示库探针残留手术）；审计刀 16 于第 80 刀后。
