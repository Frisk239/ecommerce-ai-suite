...
上一刀：**第 49 刀 源录像可改绑 + 回执指名来源**（`feat/clip-rebind-49`，closeout 见 `docs/progress/clip-rebind-closeout.md`）——解审计刀 9 的 P0-2（切片源录像绑错即锁死 + 演示稿与页面提示自相矛盾），**无新表无迁移**，修订 ADR 0047 §2（第 46 刀「已绑过的不改绑」）：新 `POST /api/clips/recordings/{id}/bind`（null=全部待拣；**可改绑已绑 pending**，已登记候选仍不动、指定即 409 且一行不写）+ `GET /api/clips/recordings`（降序 ≤20，改绑选择器数据源）+ 上传回执带后端真值 `bound_count` + 拣选回执指名「切自哪一份」；前端页头加录像下拉与「改绑」按钮（勾了就改勾选的）。验收：旧录像 256x144 上绑着 27 条 → 上传新录像 320x240 → 一键改绑 27 条 → 拣选出的资产是 **320x240 / 39.000s**（时窗 00:01:20→00:01:59），两条独立证据证明字节来自新录像。集成 892→903 passed。审计刀 9（PR #75）、第 46–48 刀（PR #72/#73/#74）与更早各刀、审计刀 6/7/8 均已合并 main。
