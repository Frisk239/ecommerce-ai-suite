...
上一刀：**第 62 刀 类目别名扩到 9/9 + 类目问句纯度闸补到库存路径**（`feat/category-alias-62`，closeout 见 `docs/progress/category-alias-expand-closeout.md`）——收审计刀 12 的 C-P2。**无表无迁移**：`CATEGORY_ALIASES` 6→18 条、覆盖 7/9 类目的口语短称（+电脑/本本、彩电、智能机、零食/吃的/食物、**书**/书籍/书本、杯子/水杯；洗衣机/家具类目名即顾客用词走字面命中，**按「顾客的说法能不能问到」覆盖 9/9**），歧义面由 `category_question_residual`（字面剔除 + 问价虚词）承担而非字面保守（「说明书/书桌/电脑包 多少钱」照旧拒答）；类目 token 改**字面剔除**修掉单字别名（「书」走 LCS 长 1 < 最小长度 2 永远剔不掉）；`category_targets` 成候选表单一真源（别名在前、类目名在后长名优先），报价与库存**共用表、各装各的闸**；**库存侧补 `stock_residual` 纯度闸**（第 56 刀只接了表没接闸）修掉「手机壳有货吗→智能手机」「电视柜→电视机」「平板支撑→平板电脑」「笔记本电脑包→笔记本电脑」4 例误答；库存侧**类目先于单品**对齐报价侧审计刀 11 P1。浏览器实测：零食/书/彩电 多少钱 → 类目价，杯子有货吗 → 器皿聚合，手机壳有货吗 → 拒答转人工（原先误答「智能手机共 10 件」）。集成 960→983 passed；ruff 全过；评测零漂移（70.0/65.0）。ADR 0037 新增第 56/62 刀修订、ADR 0045 新增二修订之三并订正写反的判定顺序。第 61 刀（PR #91）、审计刀 12（PR #90）、第 60 刀（PR #89）均已合并 main。
