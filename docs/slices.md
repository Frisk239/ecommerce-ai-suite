...
上一刀：**第 79 刀 实体亲和重排**（`docs/rerank-79`，closeout 见 `docs/progress/rerank-79-closeout.md`）——Owner 裁决「跨商品混淆必须解决」：75 刀留下的对照基线兑现为 **ADR 0048 实体亲和重排**（问句对候选资产标题的 IDF 加权 bigram 覆盖 ×3 per-query 乘数，纯词法零 LLM 零向量；实验矩阵定稿 α=3，`scripts/eval/rerank_experiment.py` 入库）。**检索基线刷新 87.5/86.2**（positive@1 72.5→87.5、overall@1 66.2→86.2、混淆@1 60.0→73.3；拒答/误拒零回归；top-1 变动 16 改善/2 变差/5 横移，变差两条均为语义歧义非硬伤）。live：75 刀两病例修复（净含量引 A-0009 正解、材质引 A-0003 进窗）。集成 1166 passed。第 78 刀（PR #113）已合并。**下一刀=第 80 刀 顾客侧「结束会话」+ 会话终态语义**（Owner 已裁决允许，48 刀 CSAT 回退点解冻）；审计刀 16 于第 80 刀后；仍待裁决：已发布探针资产 260-262/268 处置。——走查方案 §五 第 3 条的零成本半项，**纯文档刀**：①订单工具说明加「口播注意」——SO-1001 已被第 44 刀退货流真跑过、停在「退货中」，别念「已发货」（本身就是退货状态迁移素材）；②README 新增「演示路径」节——主线素材七条（#199 治理闭环 / #200/#202 多轮追问 / #203 治理写回 / #125 类目价 / #148 澄清闭环 / #198 评分可改 / #142 拼接追问，全部本会话真跑过）+ 避开清单与避开方法（demo_reset）+ 治理面「不清」的理由与用法。第 77 刀（PR #112）已合并 main。**走查方案三刀（76/77/78）全部交付**；剩 Owner 裁决项（探针资产处置、会话终态）；审计刀 16 于第 80 刀后。
