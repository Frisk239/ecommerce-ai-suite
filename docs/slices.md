...
上一刀：**第 18 刀直播切片**（`feat/clip-picking`，closeout 见 `docs/progress/clip-picking-closeout.md`）：候选种子 mock→拣选批量登记（ADR 0039：字节=带时间码转写文本、只登记视频资产不造任务 0015、拒绝原子性）→发布→检索命中；素材中心「切片汇入」纯视图页签；**含 CONTEXT.md 词条节误删事故修复**（第 17 刀脚本吞掉 -168 行，恢复+重新应用 0038 修订）。能力块 5/7。第 11–17 刀+审计刀 3 已合并 main。

当前阶段：工程刀。下一刀：**第 19 刀销售考核**（抽已发布对话出题+AI 扮客打分；词条：打的是人不是客服引擎）；审计刀 4 于第 20 刀后触发。
