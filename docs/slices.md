...
上一刀：**第 16 刀工程还债刀**（`feat/debt-1`，closeout 见 `docs/progress/debt-1-closeout.md`）：审计刀 3 P1 六项一揽子——跨循环 LLM 客户端/回流 LLM 前释放事务/库存词表收窄+商品双前置（误伤回检索留缺口）/QA 块保位/工具日志/0027 回写+P2 注释。零新功能面。第 11–15 刀+审计刀 3 已合并 main。

当前阶段：工程刀。下一刀：**第 17 刀素材中心**（任务 0012+质检过线登记 0029+material_generated+打码小面）；审计刀 4 于第 20 刀后触发。
...
| 刀 | 约束 |
| --- | --- |
| 第 17 刀素材中心 | 任务（0012 排队/进行中/失败/可重试）+ 质检过线才登记（0029）+ material_generated 来源 + 打码小面（audit-3 P1#4） |
| 第 18–20 刀 | 直播切片 → 销售考核 → 血缘视图（0014/0015/0026 已锁约束） |
| **审计刀 4** | **第 20 刀后触发** |
