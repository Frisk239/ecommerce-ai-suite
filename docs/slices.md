...
上一刀：**第 46 刀 直播切片真链路**（`feat/real-clips`，closeout 见 `docs/progress/real-clips-closeout.md`）——roadmap v3 第二梯队第二项（ADR 0047 + 迁移 0023）：新 `clip_recordings` 表（源录像**不是中台对象**：不进检索/不能发布/不进治理台）+ `clip_candidates.recording_id`；`POST /api/clips/recordings` 上传 .mp4（≤200MB）即把「尚无源录像的 pending 候选」绑上；拣选用 `ffmpeg -c copy` **真切出 mp4 片段**（失败 422 且候选保持 pending），无源录像退回时间码文本旧路径；对象键扩展名跟字节走（`.mp4`/`.txt`）；**video 正文改由 `transcript` 字段承载、永不读字节**。钉测：真切出 `ftypisom` 字节（时窗逐秒对齐）+ 发布后切块=转写原文；浏览器全链（上传→绑定 30 条→拣选→治理台→发布→块表）。集成 804→811 passed。第 45 刀（45a/45b）、第 44 刀退货状态、第 43 刀操作者仪表、第 42 刀转人工真闭环（ADR 0046 + 迁移 0020）、第 41 刀商品可运营+目录可答（ADR 0045）、第 26–40 刀+审计刀 6/7/8 均已合并 main。

当前阶段：**第三阶段产品硬ening**（施工权威 `docs/roadmap-product-hardening.md`）。**审计刀 8 已完成**（三路并行审计 **P0 全零**，P1×5 实修；证据 `docs/progress/audit-8-closeout.md`）。**第 46 刀已交付**（切片真链路，ADR 0047 + 迁移 0023）。下一刀：**第 47 刀 可观测最小版**（RED 指标 + 三个自定义 + structlog JSON + correlation-id + `/metrics`）；此后 48（CSAT + 反馈闭环）。**待 Owner 裁决**：审计刀 8 记债的四条产品面缺口（商品价 3/115、多来源在产品面不可见、工作队列首屏 183 条原始灌入、widget 不落宿主 origin）——建议在第 47 刀前后插一刀清前两条。
