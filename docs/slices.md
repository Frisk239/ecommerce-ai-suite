...
上一刀：**第 19 刀销售考核**（`feat/coaching`，closeout 见 `docs/progress/coaching-closeout.md`）：题库从已发布对话动态推导（ADR 0040：confirmed qa_pairs 逐对成题+转写首问兜底）→单轮作答→LLM 三维 rubric 打分（40/30/30，无降级可重评）→记录回放含底座名快照；coach_records 非中台对象。评审实修打分事务纪律硬违规。能力块 6/7。第 11–18 刀+审计刀 3 已合并 main。

当前阶段：工程刀。下一刀：**第 20 刀血缘视图**（0026：资产详情拼引用/写回/导出/考核使用——下游已齐可一次拼全）；**其后触发审计刀 4**。
...
| 刀 | 约束 |
| --- | --- |
| 第 20 刀血缘视图 | 资产详情拼引用/写回/导出/考核使用（0026）；运营 Agent（能力块 7/7 的最后一块）排审计后 |
| **审计刀 4** | **第 20 刀后触发**（候看：17/18/19 刀债务清单、CONTEXT 事故复盘、词表误伤真实分布、兜底题证据维语义） |

| 刀 | 约束 |
| --- | --- |
| 第 19 刀销售考核 | 抽已发布对话（QA 资产已备原料）出题+AI 扮客打分；打的是人不是客服引擎；不做上岗系统；考核导出消费面 |
| 第 20 刀血缘视图 | 资产详情拼引用/写回/导出/考核使用（0026） |
| **审计刀 4** | **第 20 刀后触发**（候看：17 刀债务七条、18 刀三条、CONTEXT 事故复盘、词表误伤真实分布） |
