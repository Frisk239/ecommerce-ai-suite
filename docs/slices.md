...
上一刀：**第 65 刀 问货/问价口语覆盖收口**（`feat/colloquial-65`，closeout 见 `docs/progress/colloquial-coverage-closeout.md`）——收第 62 刀 Spec 轴评审留下的自然问法拒答清单。**无表无迁移**：①`STOCK_KEYWORD_PATTERN`（**路由词表**）补 `卖完|卖光` + `_STOCK_FILLER_WORDS` 补原子词卖完/卖光——**两层都要**（浏览器验收抓的层间缝：只补虚词表时「书都卖完了吗」无「有」字进不了库存工具，直调 query_stock 的单测测不到路由层）；②问价意图 `PRICE_RE` 补 `咋卖/咋买/贵`（贵排在贵不贵后）+ 纯度 `_PRICE_RE` 同步补裸贵（死意图路径）；③不收「到货」（到货时间问句照旧回落拒答留缺口，正确归宿）。浏览器实测：书都卖完了吗→「图书共 19 件，其中有货 19 件，库存合计 846 件」、杯子卖光了吗→器皿聚合、手机咋卖→智能手机类目价；负例 9/9 全拒答（含退货运费贵吗——政策闸仍在意图层之前）。**两轴评审无 P0/P1、P2×5 全实修**：库存**单品路径补纯度闸**（`stock_product_residual`，此前「保温杯卖完了还能刻字吗」被硬答「有货 42 件」）；虚词补裸没/裸全/量词原子词（**字符类会被 re.escape 转义成字面量，必须逐个枚举**——实测踩到）；刀号与会话数订正。集成 1000→1017 passed；ruff 全过；评测逐位相同。第 64 刀（PR #94）、第 63 刀（PR #93）、第 62 刀（PR #92）均已合并 main。**下一刀=审计刀 13（覆盖第 61–65 刀）**。
