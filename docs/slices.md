...
上一刀：**第 66 刀 评论证据适用域（弱引用质量闸）**（`feat/review-evidence-66`，closeout 见 `docs/progress/review-evidence-scope-closeout.md`）——收审计刀 13 P0-2 根因。**动检索语义**（before/after 逐位相同 + ADR 0018 修订）：测量证明分数阈值分不开（坏 case 0.4–0.5、金标真命中最低 0.17——短评论块 bigram 少分数天然高），分得开的是证据**类别**与问句**意图**——`excludes_review_evidence`：服务状态词（到货/退货/物流…）在场且无观点标记（怎么样/值得/评价…）→ `review_import` 块不算证据；落点在 `retrieve()` 候选集（SQL 随带 source_kind），引擎/缺口新鲜度/MCP/评测全出口同语义；**观点豁免零回归**（金标 19 条期望评论资产的 case 全部带观点标记）。live 实测：到货了吗→拒答·转人工（不再引英文评论）、退货运费多少钱→拒答留缺口（政策文档确实没写运费，此前的「答案」本来就是别人的评论）、物流怎么样→照常引评论。集成 1034→1051 passed；评测 before/after 逐位相同。审计刀 13（PR #96）、第 65 刀（PR #95）均已合并 main。
