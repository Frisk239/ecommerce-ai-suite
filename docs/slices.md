...
上一刀：**审计刀 13**（`feat/audit-13`，closeout 见 `docs/progress/audit-13-closeout.md`）——三路并行只读审计第 61–65 刀（PR #91–#95）：**P0×2 实修**（①拼接检索的本问保底 `merge_own_hits`——净含量→材质追问此前稳定拒答（prompt/引用只看前两条，材质块被挤出证据窗），交错并入后 live 三连测「它的材质是钛钢。[2]」；②「到货了吗」弱引用胡答——65 刀「拒答留缺口」表述在 live 评论语料上为假，原处订正+弱引用质量闸记债）+ **P1×4**（花多少钱死意图——re 最左起始位陷阱补裸「钱」；G26/G1 陈旧缺口用产品机制关闭；README 指标演示三步化；端口口径注明随 PG_PORT）+ **P2×13**（提议步闸按原问句判、售罄断货/指示词/本书/多词表、62 刀门禁假账 +38/998 订正、ADR 0037 两刀未回写补记、demo_reset ~* 对齐+回显目标库、isPublished 手抄收口、SO-1001 口播注释订正等）。集成 1017→**1034 passed**；ruff 全过；前端 lint 7/0、build 绿；评测逐位相同（70.0/65.0，覆盖面边界在 rag-eval-report 补记）。第 65 刀（PR #95）、第 64 刀（PR #94）、第 63 刀（PR #93）均已合并 main。**下一刀=第 66 刀**（候选：弱引用质量闸（P0-2 根因，动检索需评测对照）/ 56 刀词面邻缝收口 / Owner 裁决项）。
