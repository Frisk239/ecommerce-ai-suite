...
上一刀：**第 15 刀评测集**（`feat/eval-set`，closeout 见 `docs/progress/eval-set-closeout.md`）：golden.json 13 条（引用/拒答/订单/库存/policy edge，标题锚不锚 id）+ 参数化 runner 直调 run_ask 进回归防线（ADR 0027 v1=可复现记录；产品代码零改动）。前十四刀已合并 main。

当前阶段：**计数线到——下一刀=审计刀 3**（三路子代理：设计符合性/技术债/功能缺口；不改产品代码，产出还债排期）。
...
| 刀 | 约束 |
| --- | --- |
| **审计刀 3** | **下一刀（计数线到）**；候看：工具共享缝 45 行、词表误伤面、12/13 刀债务清单、素材/切片/考核缺口 |
| 素材 / 切片 / 考核 | 审计后按缺口排。质检、候选不是资产已锁。不做微调（0028） |
