...
| 7 | 不再给 WANDS 切片候选加来源列：候选页显示 `source_video_label`（「WANDS · wayfair 家具检索基准」），四份数据在**至少一个产品面**可见即达标。**订正（审计刀 10）**：当时理由是「候选页已显示 label」，实际只把 `clips[0]` 的 label 渲染在页头（候选按 id 升序，那是种子候选）——审计刀 10 已把每张候选卡补上自己的 `source_video_label`（逐卡可见） | 候选不是中台对象（第 18 刀），加列是为对齐而加列 |
