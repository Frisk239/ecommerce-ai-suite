...
## 第 109 刀：数据清理（2026-09-17，施工单执行——资产 24 条软删 + golden 锚维护）

第七阶段第二刀：执行第 108 健康审计报告施工单（`scripts/realdata/data_cleanup.py`，
默认 dry-run、`--apply` 写库、每类一函数+汇总；重跑幂等）。数据库侧动作全表见
`docs/research/data-health-report.md` 尾部「109 执行结果」段——与本节基线相关的
两条：**资产软删 24 条**（ADR 0042：`discarded_at`，不删行不删字节；含 5 份英文
回流对话、退货主题冗余 A-6/A-13、探针冗余份）；**A-483 品牌 QID 修订发布**
（Q5019402→Fairphone，操作者 HTTP 通道开修订换字节+人洗确认+发布 v2；v3 为实施
中一次幂等判据的 API 字段名笔误多开的同字节空修订，已修，重跑 0）。

### golden 维护（被 retire 资产：期望锚迁移或移除；60/100 刀先例）

retire 清单 × `golden_large.json` cite 锚交叉核对命中 **7 条**：

- **迁移 1 条**：syn-005「请问退换政策说明值得入手吗」锚 A-6 → **A-479**
  （口径归一主份，内容覆盖全部退货口径）；
- **移除 6 条**（锚 A-223/224/225 英文对话无 keep 份）：pos-025、pos-033、
  pos-037、syn-016、syn-017、exp-pos-019。

维护后 236→**230 条**（positive 80→76、paraphrase 58→56，其余分布不变）。

### Before（维护后 golden 230 条 × 清理前；逐位=108 基线在维护后的同库值）

```
分布             条数  recall@1  recall@3     MRR   nDCG@3     噪声@3     拒答率     误拒率    混淆@1
positive       76     98.7%    100.0%  0.9934   0.9951    40.8%       -    0.0%       -
paraphrase     56     92.9%     98.2%  0.9554   0.9624    32.1%       -       -       -
confusion      25     72.0%     92.0%  0.8133   0.8409    60.0%       -       -   72.0%
refusal        30         -         -       -        -        -   86.7%       -       -
oov_syn        15     86.7%     93.3%  0.9000   0.9087    40.0%       -       -       -
sem_neg        28     78.6%     92.9%  0.8452   0.8665    35.7%       -       -       -
overall       230     90.0%     97.0%  0.9325   0.9422    40.0%   86.7%    0.0%   72.0%
```

（236 条原表同库复跑=89.8/97.1/0.9320/0.9421/39.0——与 107b 终态逐位一致，维护
只动了 7 条 case 集合。）

### After（`--apply` 清理后，同库同 golden；机械粘贴 run_eval stdout）

```
分布             条数  recall@1  recall@3     MRR   nDCG@3     噪声@3     拒答率     误拒率    混淆@1
positive       76     97.4%    100.0%  0.9868   0.9903    35.1%       -    0.0%       -
...
### 漂移逐位归属（before/after 全 case 对照，工件 `109-case-diff.txt`；同状态复跑逐位相同）

- **@1 180/200 → 175/200（−5）；@3 194/200 逐位不变**——5 条退步全是「期望资产
  第 1→第 2（仍在前 3，MRR 1.0→0.5）」：exp-syn-001「怎么退换」/exp-syn-033
  「退换要扣钱吗」/sneg-neg-005「不能退换吗」→ **A-301**（评论导入·平板评论含
  「退货」字样）抢 @1；sneg-neg-007「不支持七天无理由退货吗」→ **A-492**（刻字
  口径）抢 @1；pos-023 → **A-228**（OFF 规格）抢 @1。
- **机制（代码路径，非检索改动）**：79 刀实体亲和乘数用**全库已发布标题集的
  IDF**（`retrieval.title_idf`，随资产标题集合取快照）——退役 24 条（两份额外
  退货主题文档+5 份英文对话+探针冗余）改动标题集，「退货/退换」类 bigram 的 df
  下降、IDF 上升，A-301 这类**多处复现「退货」**的评论块相对获益，在 5 条上翻
  过期望资产。本刀零改检索代码；同库重跑（runner 与逐 case 采集各两次）逐位相同。
- **噪声@3 40.0→35.0（−5.0pp）**：退役块不再占 top-3 位（对照工件里 40 条 case
  的 top 列表，旧 A-6/A-13/A-223~225/A-484 等位置换成期望资产自身块或保留资产）
  ——清理的直接收益（负面证据面的直接扣除）。
- **双红线逐位保持**：拒答率 86.7（26/30）、误拒 0.0；confusion/oov_syn/refusal
  三分布逐位不变；positive/paraphrase @3 保持 100.0/98.2。
- 记债 **#109-P1**：退货/退换主题 3 条 @1 现被评论/刻字文档抢——66/71 刀评论闸
  与 79 刀亲和 IDF 的边界（语料变小后暴露）；转 110/111 或单开检索刀核（本刀
  范围=数据清理，不动检索权重）。

### 复现命令（仓库根目录）

```
# 清理（dry-run 默认；--apply 写库，幂等：retire 0/fix 0，缺口重问步骤对仍 open 的 8 条为 live 探针）
uv run python scripts/realdata/data_cleanup.py --db postgresql://suite:suite@localhost:5433/suite
uv run python scripts/realdata/data_cleanup.py --db postgresql://suite:suite@localhost:5433/suite --apply
...
## 第七阶段收官（第 111 刀，2026-09-17，护栏+收官——检索层零改动）

第七阶段（108 审计 → 109 清理 → 110 重灌 → 111 护栏/收官）的终刀。本刀改动面
**不含检索与生成**：机洗/人洗的图片描述复核护栏（`services/image_verify.py` +
PATCH 附注 + 前端徽章）、演示脚本（demo_prepare/demo_reset）、店铺宿主页（W1）。
基线复跑确认终态，`#109-P1` 做最终归属定论。

### 治理前后基线链（golden 230 条；同库同集合）

| 阶段 | 时点 | recall@1 | recall@3 | MRR | nDCG@3 | 噪声@3 | 拒答率 | 误拒率 | 混淆@1 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 治理前（236 条原始 golden，107b 终态） | 第五阶段收官 | 89.8% | 97.1% | 0.9320 | 0.9421 | 39.0% | 86.7% | 0.0% | 72.0% |
| 109 before（230 条维护后、清理前） | 第七阶段起点 | 90.0% | 97.0% | 0.9325 | 0.9422 | 40.0% | 86.7% | 0.0% | 72.0% |
| 109 after（软删 24 条 + 口径归一） | 清理交付 | 87.5% | 97.0% | 0.9200 | 0.9330 | 35.0% | 86.7% | 0.0% | 72.0% |
| 110 after（真图重灌 + 转写重登记） | 重灌交付 | 88.5% | 97.0% | 0.9250 | 0.9367 | 35.0% | 86.7% | 0.0% | 72.0% |
| **111 终态（护栏+收官，检索零改动）** | 本刀 | **88.5%** | **97.0%** | **0.9250** | **0.9367** | **35.0%** | **86.7%** | **0.0%** | **72.0%** |

**一句话判定**：清理的语料收缩代价（90.0→87.5，@1 −2.5pp，全部来自标题 IDF/
亲和重排，非检索缺陷）在重灌真内容后**部分自然恢复**（87.5→88.5，两条 sem_neg
回 @1）；剩余 5 条 @1 退步与清理时的 robber 一字不差（见下 P1 定论）——终态
88.5/97.0 已含双红线（拒答 86.7 / 误拒 0.0）逐位保持，第七阶段验收基线以此为准。

### 终态基线（本刀 run_eval stdout 原样，230 条）
...
- 复现性：runner 连跑两遍 stdout 全表**逐位相同**；逐 case 采集两次对比 230 条
  里 229 条全同，唯 conf-012 的**并列尾部**（rank2/3 两条非期望块同分位次互换，
  expected A-29 仍 @1）不同——该 case 全部指标两次相同，属同分块的稳定排序面，
  不影响任何指标格（如实记，不粉饰）。

### #109-P1 最终归属定论（五条 @1 退步，本刀结项）

五条 = exp-syn-001「怎么退换」/ exp-syn-033「退换要扣钱吗」/ sneg-neg-005
「不能退换吗」（均被 **A-301** 评论导入块抢 @1）、sneg-neg-007「不支持七天
无理由退货吗」（被 **A-492** 刻字口径抢 @1）、pos-023（被 **A-228** OFF 规格
抢 @1）。本刀逐 case 复核（工件 `111-cases-final.json`）：五条**期望资产均在
rank2、MRR 0.5、robber 与 109/110 一次字不差**——109 清理后如此，110 重灌后
如此，111 终态仍如此。

**定论（不再修检索代码）**：这是**语料规模效应**而非检索缺陷——79 刀实体亲和
乘数用全库已发布标题集的 IDF（`retrieval.title_idf`，随标题集合取快照），
109 退役 6 条英文锚回流对话 + 退货主题冗余份后标题集变小，「退货/退换」类
bigram 的 df 下降、IDF 上升，A-301 这类多处复现「退货」的评论块与 A-492 刻字
口径相对获益，在 5 条上翻过期望资产；且 A-301 是**真实已发布评论证据**（66/71
刀评论闸的适用域边界在此暴露），扣它属于检索口径裁决不是数据错误。
**后续语料扩容可自然恢复**（标题集变大 → 该 bigram IDF 回落）；若未来语料
稳定后仍复现，则单开「评论块 vs 口径文档」检索闸刀重新裁决——第七阶段不为其
改代码（本刀检索层零改动，基线逐位不变）。

### #109-P2 无残留确认（旧转移写）

- 库内磁盘面：`asset_versions.extracted_fields/confirmed_fields` 与
  `retrieval_chunks` 全文扫「已发布资产里没有」= **0 命中**；
- A-14/A-270（旧）与实施副产物 A-518/519 均 `discarded_at` 非空；A-520（转写止于
  「客服：瓶装水暂时无货。」，机洗 qa_pairs 3 对）与 A-521（纯顾客问句、机洗弃权）
  在位且未发布（不进检索）；
- 判定：**#109-P2 结项，无残留**。
...
```
# 终态基线与逐 case 采集（230 条）
uv run python scripts/eval/run_eval.py --db postgresql://suite:suite@localhost:5433/suite
uv run python .scratch/cases_110.py scripts/eval/out/111-cases-final.json

# 图片描述护栏四态（替身 VLM；真 PG 测试库）
SUITE_TEST_DATABASE_URL=postgresql://suite:suite@localhost:5433/suite_test \
  uv run pytest apps/api/tests/test_image_verify_guard.py -q

# 护栏真 VLM 核验（已发布资产字节 + 隔离测试库 HTTP 全链，不写演示库）
uv run python .scratch/verify_111_guard_real_vlm.py
```

工件：`out/111-eval-final.txt` / `out/111-eval-rerun.txt`（两次 stdout）、
`scripts/eval/out/111-cases-{final,rerun}.json`（逐 case）、
`out/111-junit.xml` + `out/111-pytest.txt`（全量门禁）、
`out/111-vlm-guard.txt`（护栏真 VLM 核验 + 厂商 400 时的 fail-open 实测）、
`out/111-demo-prepare.txt` / `out/111-demo-reset.txt`（演示就绪与清理盘点）、
`out/111-shot-storefront.png`（W1 店铺页）、`out/111-shot-badge-{ok,warn}.png`
（护栏徽章两态的真实组件渲染，路由替身不改库）。

### W5 剩余债表状态（第七阶段收官时点，全部如实挂账不藏）

| 债 | 来源 | 终态现状 | 处置 |
| --- | --- | --- | --- |
| #109-P1 五条 @1 退步 | 109 清理节 | 本刀复核：robber 一字不差（A-301×3 / A-492 / A-228） | **结项定论**（见上：语料扩容自然恢复；再现则单开检索闸刀） |
| oov_syn 探针量 15（未扩 30+） | 107b | 15 条已撑起表外泛化裁决（86.7/93.3） | 挂账：长尾扩容待有真实改写样本时再做（不造数据凑量） |
| sem_neg 6 miss（nb-003/004/005/006 + neg-005/007） | 107a/109 | nb 四条（480/492 被 A-9 短块、268 抢 @1）+ neg 两条=P1 同源 | 挂账：实体×意图交叉的稠密/重排联合项——向量面已具备（106 融合），留给需要它的刀 |
| conf-007 @3 未中（0/0） | 107b | 期望 A-226 未进前 3（230/231/232 同族规格互抢） | 挂账：混淆族同题多份的排序裁决（数据面可合并冗余份） |
| 图片描述 vs 画面靠启发式（关键词交集） | 111 护栏 | 真 VLM 实测：A-501 交集 15、HTTP 全链 13（一致）；厂商 400/超时=fail-open | 已产品化（护栏四态 + 徽章）；不追求自动否决（治理权在人） |
