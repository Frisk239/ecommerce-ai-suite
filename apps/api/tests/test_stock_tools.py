...
def _stub_ticket(monkeypatch: pytest.MonkeyPatch) -> None:
    """第 108B 刀（W4）：拒答消息文本要格式化工单号 H-xxxx——真函数走库拿 int
    PK，MagicMock 会话给不出 id，用替身（与 27 刀 record_refusal_gap 替身同款；
    号码位的文本由 test_answer 单测 + test_service_integration 真库钉）。"""
    monkeypatch.setattr(
...
def test_run_ask_narrowed_words_never_touch_stock_tool(
    ...
    monkeypatch.setattr("suite_api.services.chat_engine.record_refusal_gap", lambda *_a, **_k: None)
    _stub_ticket(monkeypatch)  # 第 108B 刀 W4：拒答文本还要格式化工单号
