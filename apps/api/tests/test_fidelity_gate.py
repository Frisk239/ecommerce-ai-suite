...
@pytest.mark.parametrize(
    ...
    [
        ...
        # 审计刀 16 C-P0-1：裸覆盖标记句（无证据类主语）同样按免责收口——
        # live 实测「[未覆盖]」带引用以 kind=answer 漏给顾客（~8% 复现）
        ("[未覆盖]", ""),
        ("无法确认。[1][2]", ""),
        ("暂无相关信息。", ""),
        ("条码是 000000000063。[1] 未覆盖。", "条码是 000000000063。"),
        # 负例：事实否定句含「不包含」但整句有实质内容 -> 不许被误摘（刀 12 口径）
        ("配料信息中不包含任何防腐剂。[2]", "配料信息中不包含任何防腐剂。[2]"),
    ],
)
