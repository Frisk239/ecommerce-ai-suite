...
def resolve_gap_answered_by_catalog(db: Session, question: str) -> bool:
    """答上了 -> 同问的 open 缺口收掉（第 56 刀目录路径；审计刀 12 起**所有出口共用**）。

    为什么：缺口是「知识待补」的待办。问句**当下已经能答**（目录回落、库存事实、
    订单工具、RAG 生成）时它就不再需要补文档；挂着会让治理台的「待补」写着客服
    已经能答的问题（点「去补文档」还会把人引去写一份不需要的文档）。审计刀 12
