...
PRICE_RE = re.compile(
    ...
    # 第 65 刀：补「咋卖/咋买」（怎么卖 的北方式说法，评审实测只认「怎么」不认
    # 「咋」）与裸「贵」（「手机贵吗」——必须排在「贵不贵」后面，先匹配长词，
    # 否则「贵不贵」会被「贵」吃掉剩「不」）。
    # 第 67 刀：咋样卖/咋样买（北方「怎么卖」的又一体，65 刀只收了咋卖）、咋价
    # （与啥价对称——「这手机咋价」此前意图层都不进）。咋样**单独**不是价格词
    # （「这个咋样」是评价问），只收复合词。
    "怎么卖|怎么买|卖多少|什么价|啥价|多钱|咋卖|咋买|咋样卖|咋样买|咋价|贵"
)
...
# 都是自然问价）。第 65 刀补裸「贵」（意图层加了「贵吗」形态，纯度层剔不掉裸
# 「贵」就是死意图路径——排在「贵不贵」后面，长词先匹配）。
# 审计刀 13 B 轴 P1：「花多少钱」是死意图——re 择先按**最左起始位置**，不按书写
# 顺序：「花多少」比「多少钱」早一个字符起吃，剩下「钱」不是原子词，整句被挡。
# 长词在前管不住这种前缀起始更早的情形，补裸「钱」兜底（「押金多少钱」的「押金」
# 仍会挡住，不误放）。同批补「我想」（「我想买保温杯多少钱」）。第 70 刀评审
# 顺带补 帮/我/查（「帮我查下手机多少钱」——礼貌动词；真修饰语靠残字挡）。第 67 刀补
# 裸 这/个/台（「这手机啥价」「那台笔记本电脑多少钱」的指示词残字；「那」此前
# 已在表里——复审审计去掉了重复项）。
_PRICE_RE = re.compile(
    "多少钱|钱|贵不贵|贵|花多少|怎么样|咋样|"
    "价格|售价|定价|报价|收费|便宜|请问|问一下|多少|多钱|什么|怎么|怎样|如何|"
    "你好|您好|想问|我想|麻烦|问下|一下|下|价|帮|我|查|"
    "这个|这款|这件|你们|咱们|的|是|要|买|卖|款|啥|咋|这|那|个|台|"
    "一[台本部个包盒件套杯双]|"
    r"[的吗呢啊嘛呀吧呗哦噢啦]|、|，|？|\?|！|!|。| |　"
)
